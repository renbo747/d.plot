...
//        dadaEventMapper.updateReadCnt(params.getStr("eventidx"));
		...
		List<SOMap> conddataList = dadaEventMapper.inquireEventCond(params);

		eventDtl.put("conddata", conddataList);
    ...
    public void insertPromotion(SOMap params, Map<String, MultipartFile> files) throws Exception {
        ...
        try {
			...
			System.out.println("000000111111111111111params:" + params);
        	//추첨확률조건 추가
			if (!Util.isEmpty(params.get("conddata"))) {
				System.out.println("111111111111111111 conddata not nulllllll --------------------");
				List condList = (List) params.get("conddata");
				if (condList.size() > 0) {
					System.out.println("22222222222 conddata :" + condList.size());

					for (int i = 0; i < condList.size(); i++) {
						Map conddata = (Map) condList.get(i);
						conddata.put("eventidx", eventIdx);
						dadaEventMapper.insertEventCond(conddata);
					}
				}
        	}
    ...
    public void updatePromotionDetail(SOMap params, Map<String, MultipartFile> files) throws Exception {
        ...
        try {
			...
			dadaEventMapper.updateEventCondTrash(params);
	...
	public Response updatePromotionLotteryinit(SOMap params) {
		Response res = new Response();

		params.put("issucc", "F");
		dadaEventEnterMapper.updateAdminEventEnterArray(params);

		params.put("isselectcomplete", "F");
		dadaEventMapper.updateEvent(params);
		return res;
	}
	...
	public Response updatePromotionLottery(SOMap params) {
		...
		if(!"T".equals(resultWinning.get("isselect"))) {
			res.setStatusCode(Status.CREATED.getKey());
			res.setMessage("추첨 이벤트가 아닙니다.");
			return res;
		}
		...
		// 추첨 조건에 따른 확률 결과 저장
		// (혹시나) 이미 조건이 저장되있다면 삭제 처리(ISTRASH = 'T')
		dadaEventMapper.updateEventCondResultTrash(params);


		List<SOMap> eventCondList = dadaEventMapper.inquireEventCond(params);

		if (eventCondList != null && eventCondList.size() > 0) {
			for (int i = 0; i < eventCondList.size(); i++) {
				SOMap eventCode = eventCondList.get(i);
				eventCode.put("eventidx", params.get("eventidx"));
				try {
					if ("A".equals(eventCode.get("condcode"))) { // A: 장바구니
						dadaEventMapper.insertEventCondResultA(eventCode);
					} else if ("B".equals(eventCode.get("condcode"))) { // B: 주문(결제완료)
						dadaEventMapper.insertEventCondResultB(eventCode);
					} else if ("C".equals(eventCode.get("condcode"))) { // C: 결재금액
						dadaEventMapper.insertEventCondResultC(eventCode);
					} else if ("C".equals(eventCode.get("condcode"))) { // D: WISHLIST
						dadaEventMapper.insertEventCondResultD(eventCode);
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
...
//		List winnerList =  dadaEventMapper.selectLotteryWinner(params);
		...
		for (int i = 0; i < (int) params.get("winnercount"); i++) {
			Integer winnerIdx = dadaEventMapper.selectLotteryWinnerOne(params);

			if (winnerIdx != null) {
				winnerList.add(winnerIdx);
			}
			params.put("list", winnerList);
		}
...
//		}
	 ...
	 * @return
	 */
	public Response raffleEntryList(SOMap param) throws Exception {
		Response res = new Response();
		SOMap resultMap = new SOMap();
//		if(!isMember()) {
//			res.setStatusCode(Status.FAIL.getKey());
...
//		}
		...
		List<SOMap> entrylist = dadaEventMapper.raffleEntryList(param);
