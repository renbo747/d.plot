...
public class AdminCouponServiceImpl extends MallBaseService implements AdminCouponService {
     ...
     * @throws Exception
	 */
    public SOMap getDownUseCount(SOMap params) throws Exception {
    	params.put("siteid", cs.getStr("siteid"));
        SOMap result = new SOMap();
        if(params.getStr("comcpntype").equals("CCT001") && params.getStr("isautopay").equals("T")) { 
        	result.put("result", couponMapper.getDownGoodsUseCount(params));
        } else {
        	result.put("result", couponMapper.getDownUseCount(params));
        }
    	return result;
    }
}
