...
export default {
    ...
    isAndroid() {
        var check = false;
        var UserAgent = navigator.userAgent.toLowerCase(); //userAgent 체크
        this.debug(UserAgent.toString);
     ...
     **********************************/
    priceCutting(price, type, cutprice) {
        var removePrice = 0;
        type = type ? type : "R";
        removePrice = price / cutprice;
        if (price <= cutprice) {
            return price;
        }
        if (type == "F") {
            removePrice = Math.floor(removePrice);
        } else if (type == "R") {
            removePrice = Math.round(removePrice);
        } else if (type == "C") {
            removePrice = Math.ceil(removePrice);
        }
        removePrice = removePrice * cutprice;
        return removePrice;
    },
    /**********************************
     * 전화번호 keyup -(하이픈) 이벤트
