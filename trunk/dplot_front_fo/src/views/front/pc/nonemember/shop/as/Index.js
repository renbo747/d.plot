...
export default {
    mounted() {
        //as-신청으로 접근
        if(this.$route.name == 'nonemember-apply-as'){
            var param = {
            ...
            //as-수정으로 접근
            if(this.$route.name == 'nonemember-apply-as-revise'){
    ...
    methods: {
         ...
         ****************************/
         onInit(param){
            ...
            //1:1 문의 데이터 로드
            this.$http.post('/nonemember/as/apply', param).then(result => {
                if (result.statusCode == 200) {
                    if(result.data.auth == 'success'){
                        ...
                        this.product.delivcnt = result.data.list.asordcnt;
         ...
         ****************************/
         applyComplt() {
            ...
            var param = {
                ...
                addrdetailroad : this.address.detail,
            ...
            }else if(this.$route.name == 'nonemember-apply-as-revise') {
        ...
        *********************/
         saveAsApply(param) {
            this.$http.post('/nonemember/as/applysave',param).then(result => {
                if(result.statusCode == 200){
        ...
        *********************/
        saveRevise(param){
            this.$http.post('/nonemember/as/applyrevise',param).then(result => {
                if(result.statusCode == 200){
                    if(result.data.msg == 'success'){
                        this.$eventBus.$emit('alert', '확인' ,"수정이 완료되었습니다.");
                        this.$router.push({ name: 'nonemember-as-detail', params:{pid:this.$route.params.pid}});
                    }
                }
            })
        },
        ...
        *********************/
        sendKakao(param) {
            this.$http.post('/nonemember/as/sendapply',param).then(result => {
            })
        },
        /*********************************
         * 첨부파일 변경시 처리
