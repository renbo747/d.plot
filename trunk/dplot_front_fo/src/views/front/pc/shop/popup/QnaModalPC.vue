<template>
  <b-modal
    id="QnaModalPC"
    modal-class="dp-modal page-layer product-inquiry-modal"
    centered
    scrollable
    :hide-footer="true"
  >
    ...
    <template #modal-header="{ cancel }">
      <h5 class="modal-title">상품문의하기</h5>
      ...
      <div class="page-layer__body">
        <div class="dp-mb-30">
          ...
          <form>
            <base-dropdown
              placeholder="선택해주세요."
              v-model="searchData.qnatype"
              :options="qnatypeOptions"
            />
          </form>
        </div>
        <div v-for="(list, index) in optionList" :key="index" class="dp-mb-30">
          <p v-if="index == 0" class="dp-p font-weight-500 dp-mb-10">
            상품옵션 (선택사항)
          </p>
          <form>
            <base-dropdown
              ...
              :options="list.detail"
              @change="selectOption(list, index, list.selected)"
              :key="skey"
            />
          </form>
        </div>
        <div class="dp-mb-30">
          <p class="dp-p font-weight-500 dp-mb-10">문의내용</p>
          <form>
            <base-textarea
              class="textarea-h-140"
              v-model="searchData.content"
              placeholder="반품/환불 문의는 1:1문의로 가능합니다."
            />
            <base-checkbox
              label="비공개 문의 작성"
              name="privateInquiryCheck"
              :id="privateInquiryCheckbox.id"
              trueValue="T"
              falseValue="F"
              v-model="searchData.issecret"
            />
          </form>
        </div>
        <div class="btn-group">
      ...
      this.$http.post("/cscenter/option", { goodsno: this.searchData.goodsno }).then((result) => {
          if (result.statusCode == 200) {
            ...
            for (var i = 0; i < result.data.optionlist.length; i++) {
              this.optionList[i].selected = this.optionList[i].optionname;
              this.optionList[i].placeholder = this.optionList[i].optionname;
