<template>
  <!-- Todo: 적립금 D포인트 한페이지에 있음-->
  <main class="dp-mypage-reward-pc">
    <div class="container">
      <div class="mypage-reward__header">
        <p class="mb-0" v-if="$route.name == 'mypage-reward'">적립금</p>
        <p class="mb-0" v-else-if="$route.name == 'mypage-employee-reward'">임직원 적립금</p>
        <p class="mb-0" v-else>D포인트</p>
          ...
          <div class="reward__box-area">
            <div class="use-reward__box">
              <div class="box__left">
                ...
                <p class="mb-0 use-reward__p" v-else-if="$route.name == 'mypage-employee-reward'">사용가능 임직원적립금</p>
            ...
            <div class="dp-bar__area" v-if="extinctpoint > 0">
              <div class="dp-bar"></div>
            </div>
            <div class="extinction-reward__box">
              <template  v-if="extinctpoint > 0">
      ...
      <!-- 임직원 회원일 때-->
      <section class="reward-use__guide__section" v-else>
        <div class="reward-use__guide__body">
          <div class="reward__box-area">
            <div class="use-reward__box pb-0">
              <div class="box__left">
                <i class="dp-icon icon-money h40"></i>
                <p class="mb-0 use-reward__p">사용가능 임직원적립금</p>
              </div>
              <p class="reward-price__area mb-0 price__area">
                <span class="text-primary atten-new">{{$util.maskComma(userpoint)}}</span>
      ...
      <section class="promotion-code__section" v-if="$route.name != 'mypage-employee-reward'">
        ...
        <div class="promotion-code__body d-flex justify-content-between align-items-end">
          ...
          <!-- D포인트 -->
          <div class="reward-use__guide__header__area d-flex" style="cursor: pointer; " v-if="$route.name == 'mypage-epoint'" @click="openModal('epointModal')">
            <i class="dp-icon icon-info md"></i>
            <p class="mb-0 reward-use__guide__header__title">
              D포인트 사용안내
            </p>
          </div>
          <!-- 적립금 -->
          <div class="reward-use__guide__header__area d-flex" style="cursor: pointer;" v-if="$route.name == 'mypage-reward'" @click="openModal('rewardInfoModal')">
            <i class="dp-icon icon-info md"></i>
      ...
      <section class="reward-content__section">
        ...
        <div v-if="$route.name != 'mypage-employee-reward'">
          <ul class="dp-sort__ul type02 scroll list-style-none no-scrollbar">
            <li v-for="(list, index) in pointType" :key="index">
              <div>
                <label :for="list.id" class="dp-sort__label" style="cursor: pointer;">
                  <input
                    type="radio"
                    :id="list.id"
                    name="reward"
        ...
        <div class="reward-content__area">
          ...
          <!-- 적립금 & D포인트 내역이 없을 때-->
          <div v-if="isEnd && rewardList.length <= 0">
            <div class="empty__area">
              ...
              <div class="empty__body d-flex flex-column align-items-center">
                <div class="empty-icon-area d-flex align-items-center justify-content-center">
                  <i class="dp-icon icon-point h68" v-if="$route.name == 'mypage-epoint'"></i>
                  <i class="dp-icon icon-reward h68" v-else></i>
                </div>
                <div class="empty-text__area">
                  <p class="empty-text mb-0" v-if="$route.name == 'mypage-epoint'">
                    D포인트 내역이 없어요
                  </p>
                  <p class="empty-text mb-0" v-else-if="$route.name == 'mypage-reward'">
                    적립금 내역이 없어요
                  </p>
                  <p class="empty-text mb-0" v-else>
                    임직원적립금 내역이 없어요
                  </p>
                </div>
              </div>
