<template>
    ...
    <div class="content m-leftmenu">
        ...
        <div class="inner">
            <div class="clearfix">
                <div class="bar-title fl">입금대기</div>
            </div>
                ...
                <table cellpadding="0" cellspacing="0" class="data-tb align-c" :style="!isPartner? 'width: 2575px;':''">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th v-if="!isPartner">입금자
                                <button type="button" class="sort" :value="sortData.vircusname"
                                    :class="[{up : sortData.vircusname === 'vircusname_asc'}, {down : sortData.vircusname === 'vircusname_desc'}]"
                                    @click="sortToggle(sortData.vircusname)"></button>
                            </th>
                            <th>결제수단
                    ...
                    <tbody v-if="list.length > 0">
                        <tr v-for="item in list" :key="item.orgdelividx">
                            <!-- <td><input type="checkbox" :id="item.orgdelividx" v-model="checkedList" :value="item.orgdelividx" @change="checkList($event.target.checked)"/></td> -->
                    ...
                    <tbody v-else>
                        <tr><td :colspan="isPartner? 17:29">조회 결과가 존재하지 않습니다.</td></tr>
                    </tbody>
                </table>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            searchData: {
                ...
                ordstatusArr: [],           // 조회할주문상태 (입금대기, 입금전취소)   
                skeyArr: [
            ...
            sortData: {
                ...
                vircusname: 'vircusname_asc',   // 입금자명
    ...
    methods: {
        ...
        onInit() {
            ...
            this.searchData.ordstatusArr = [this.$store.getters['ADMIN'].ORDER_WAITING_DEPOSIT, this.$store.getters['ADMIN'].ORDER_CANCEL_BF_DEPOSIT];
            // 공통코드 조회
        ...
        fnExcelDownload: function() {
            ...
            let params = this.searchData;
            params.isPartner = this.isPartner;
            params.fileName = '입금대기_주문목록.xlsx';
