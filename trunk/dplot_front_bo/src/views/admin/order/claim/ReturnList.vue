<template>
    ...
    <div class="content m-leftmenu">
        ...
        <div class="inner">
            <div class="clearfix">
                <div class="bar-title fl">반품관리</div>
            </div>
            <div class="boxing search-area pd0">
                ...
                <dl>
                    ...
                    <dd>
                        <select v-model="searchData.dtkey">
                            <option value="clmreq">반품신청일</option>
                            <option value="clmcomp">반품완료일</option>
                ...
                <dl>
                    ...
                    <dd>
                        <div class="check-wrap">
                            <input type="checkbox" id="chkAllRtnstatus" v-model="searchData.isallrtnstatus" true-value="T" false-value="F" @change="checkAllRtnstatus">
                            <label for="chkAllRtnstatus">전체</label>
                        </div>
                        <div class="check-wrap" v-for="item in commonCode.rtnstatus1" :key="item.cmcode">
                            <input type="checkbox" :id="'rtnstatus_'+item.cmcode" v-model="searchData.rtnstatusArr" :true-value="[]" :value="item.cmcode"/>
                            <label :for="'rtnstatus_'+item.cmcode">{{ item.codename }}</label>
                        </div>
                    </dd>
                </dl>
                <dl>
                    <dt></dt>
                    <dd>
                        <div class="check-wrap" v-for="item in commonCode.rtnstatus2" :key="item.cmcode">
                            <input type="checkbox" :id="'rtnstatus_'+item.cmcode" v-model="searchData.rtnstatusArr" :true-value="[]" :value="item.cmcode"/>
                            <label :for="'rtnstatus_'+item.cmcode">{{ item.codename }}</label>
                        </div>
                    </dd>
            ...
            <div class="caption-group mt10 clearfix">
                ...
                <div class="btn-group fr">
                    <button type="button" v-if="isWrite" class="btn blue-line" @click="fnChangeRtnstatus($store.getters['ADMIN'].RETURN_APPROVAL_COMPLETE, '반품승인')">반품승인</button>
                    <button type="button" v-if="isWrite && !isPartner" class="btn red-line" @click="fnChangeRtnstatus($store.getters['ADMIN'].RETURN_APPROVAL_REFUSE, '반품반려')">반품반려</button>
                    <button type="button" v-if="isWrite && isPartner" class="btn red-line" @click="fnChangeRtnstatus($store.getters['ADMIN'].RETURN_REQUEST_REFUSE, '반품반려승인요청')">반품반려승인요청</button>
                    <!-- <button type="button" v-if="isWrite" class="btn red-line" @click="fnChangeRtnstatus($store.getters['ADMIN'].RETURN_CANCEL, '반품철회')">반품철회</button> -->
                    <button type="button" v-if="isWrite" class="btn black-line" @click="fnChangeRtnstatus($store.getters['ADMIN'].RETURN_COMPLETE_DELIVERY, '회수완료')">회수완료</button>
                    <button type="button" v-if="isWrite" class="btn red-line" @click="fnChangeRtnstatus($store.getters['ADMIN'].RETURN_A_RETURN, '반송처리')">반송처리</button>
                    <button type="button" v-if="isWrite" class="btn black-line" @click="fnChangeRtnstatus($store.getters['ADMIN'].RETURN_COMPLETE, '처리완료')">처리완료</button>
            ...
            <div class="scroll-x">
                <table cellpadding="0" cellspacing="0" class="data-tb align-c" :style="isPartner? 'width: 1900px;': 'width: 2400px;'">
                    <caption>반품 목록</caption>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>반품금액
                                <button type="button" class="sort" :value="sortData.clmamt"
                            ...
                            <th>반품상태
                                <button type="button" class="sort" :value="sortData.rtnstatus"
                                    :class="[{up : sortData.rtnstatus === 'rtnstatus_asc'}, {down : sortData.rtnstatus === 'rtnstatus_desc'}]"
                                    @click="sortToggle(sortData.rtnstatus)"></button>
                            </th>
                            <th>주문번호
                    ...
                    <tbody v-if="list.length>0">
                        <tr v-for="item in list" :key="item.clmgdidx">
                            ...
                            <td><a href="javascript:void(0);" class="link" @click="goReturnDetail(item)">{{ item.clmno }}</a></td>
                            ...
                            <td>{{ $util.maskComma(item.bfordcnt) }}</td>
                            ...
                            <td class="right">{{ $util.isNull(item.clmamt)? '' : $util.maskComma(item.clmamt) }}</td>
                            <td>{{ item.clmreqtypename }}</td>
                            <td><a href="javascript:void(0);" class="link" @click="goClaimHistory(item.clmgdidx)">{{ item.rtnstatusname }}</a></td>
                            <td><a href="javascript:void(0);" class="link" @click="goOrderDetail(item.ordno)">{{ item.ordno }}</a></td>
                        </tr>
                    </tbody>
                    <tbody v-else>
                        <tr><td :colspan="isPartner? 18 : 27">조회 결과가 존재하지 않습니다.</td></tr>
                    </tbody>
                </table>
            </div>
        ...
        <ReturnDetail v-if="isShowReturnDetail" :activeClmParam="activeClmParam" :activeOrderInfo="activeOrderInfo" @closeDetail="closeReturnDetail"></ReturnDetail>
...
import ReturnDetail from '@views.admin/order/claim/ReturnDetail.vue';
...
export default {
    ...
    data() {
        return {
            ...
            searchData: {
                ...
                gdkey: '',              // 상품검색KEY
                gdword: '',             // 상품검색어
                dtkey: 'clmreq',        // 조회일자KEY (default: 반품신청일)
                period: '',             // 조회일자기간
                startdate: '',          // 조회시작일자
                enddate: '',            // 조회종료일자
                isallclmreqtype: '',    // 전체신청구분여부
                clmreqtypeArr: [],      // 신청구분Array
                isallrtnstatus: '',     // 전체반품상태여부
                rtnstatusArr: [],       // 반품상태Array
                dealerno: '',           // 입점업체번호
                psort: 'clmreqdate_desc'  // 정렬조건 (default: 최근반품신청일)
            },
            commonCode: {
                clmreqtype: [],         // 클레임신청구분
                rtnstatus: [],          // 반품상태
            ...
            isShowReturnDetail: false,   // 클레임반품상세 팝업 노출여부
    ...
    methods: {
        ...
        initSearchData: function () {
            this.searchData.clmtype = this.$store.getters['ADMIN'].CLM_RETURN;
            this.searchData.skeyArr = [
                ...
                {key: 'clmno', name: '반품번호', isShowPartner: true}
            ];
            ...
            this.searchData.isallrtnstatus = 'T';

            this.checkAllRtnstatus();
        ...
        getCommonCodeList: function() {
            let cmclassArr = ['CLMREQTYPE', 'RTNSTATUS'];
            this.$http.post('/common/code/map/list', {cmclass : cmclassArr, isloading: false})
                .then(result =>{
                    ...
                    // 반품상태 2Row로 분리
                    this.commonCode.rtnstatus1 = this.commonCode.rtnstatus.splice(0, 5);
                    this.commonCode.rtnstatus2 = this.commonCode.rtnstatus;
                    // 검색조건 초기화
                    this.initSearchData();
                    ...
                    if(this.isLink){
                      ...
                      this.searchData.rtnstatusArr = linkParam.rtnstatus;
                    }
                    // 목록 조회
                    this.searchList();
                })
        ...
        // 조회조건 - 반품상태 전체 체크
        checkAllRtnstatus: function() {
            let isAllCheck = this.searchData.isallrtnstatus;
            this.searchData.rtnstatusArr = [];
            if (isAllCheck == 'T') {
                for(let type of this.commonCode.rtnstatus1){
                    this.searchData.rtnstatusArr.push(type.cmcode);
                }
                for(let type of this.commonCode.rtnstatus2){
                    this.searchData.rtnstatusArr.push(type.cmcode);
                }
            }
        },
        ...
        // 반품상태 변경
        fnChangeRtnstatus: function(reqStatus, reqName) {
            ...
            switch(reqStatus) {
                // 반품승인(승인대기, 결제완료)
                case this.$store.getters['ADMIN'].RETURN_APPROVAL_COMPLETE:
                    possibleStatus = [
                    ...
                    break;
                // 반품반려(승인대기, 결제대기, 결제완료)
                case this.$store.getters['ADMIN'].RETURN_APPROVAL_REFUSE:
                    possibleStatus = [
                        this.$store.getters['ADMIN'].RETURN_WAITING_APPLY,
                        this.$store.getters['ADMIN'].RETURN_COMPLEATE_PAYMENT,
                    ...
                    break;
                // 반품반려승인요청(승인대기, 결제완료)
                case this.$store.getters['ADMIN'].RETURN_REQUEST_REFUSE:
                    possibleStatus = [
                        this.$store.getters['ADMIN'].RETURN_WAITING_APPLY,
                        this.$store.getters['ADMIN'].RETURN_COMPLEATE_PAYMENT
                    ];
                    ...
                    break;
                // 반품철회(결제대기)
                // case this.$store.getters['ADMIN'].RETURN_CANCEL:
                //     possibleStatus = [
                //         this.$store.getters['ADMIN'].RETURN_WAITING_DEPOSIT
                //     ];
                ...
                //     break;
                // 회수완료(회수진행)
                case this.$store.getters['ADMIN'].RETURN_COMPLETE_DELIVERY:
                    possibleStatus = [
                        this.$store.getters['ADMIN'].RETURN_IN_DELIVERY
                    ];
                    possibleStatusStr = '반품처리(회수진행)';
                    break;
                // 반송처리(회수완료)
                case this.$store.getters['ADMIN'].RETURN_A_RETURN:
                    possibleStatus = [
                    ...
                    break;
                // 처리완료(회수완료)
                case this.$store.getters['ADMIN'].RETURN_COMPLETE:
                    possibleStatus = [
                        this.$store.getters['ADMIN'].RETURN_COMPLETE_DELIVERY
                    ];
                    possibleStatusStr = '반품처리(회수완료)';
            ...
            for (let i=0; i<this.list.length; i++) {
                for (let j=0; j<this.checkedList.length; j++) {
                    ...
                    if (this.list[i].clmgdidx === clmgdidx) {
                        let rtnstatus = this.list[i].rtnstatus;
                        if (!this.$util.isNull(possibleStatus) && possibleStatus.length > 0 && possibleStatus.indexOf(rtnstatus) < 0) {
        ...
        // 클레임상세 팝업 닫기
        closeReturnDetail: function(isreload) {
    ...
    watch: {
        ...
        // 반품상태
        'searchData.rtnstatusArr': function(value) {
            if (value.length < this.commonCode.rtnstatus1.length + this.commonCode.rtnstatus2.length) {
                this.searchData.isallrtnstatus = 'F';
            } else {
                this.searchData.isallrtnstatus = 'T';
            }
        },
