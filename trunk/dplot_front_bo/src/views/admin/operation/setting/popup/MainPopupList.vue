<template>
    <div class="content m-leftmenu">
        ...
        <MainPopupRegist v-if="showRegist" v-bind:commonCodeList="commonCode" v-on:closeRegist="closeRegist"></MainPopupRegist>
        <MainPopupDetail v-if="showDetail" :commonCodeList="commonCode" :popidx="idx" v-on:closeDetail="closeDetail"></MainPopupDetail>
        ...
        <div class="inner">
                ...
                <dl>
                    ...
                    <dd>
                        <select v-model="searchData.stype">
                            <option value="">전체</option>
                            <option value="popsttime">시작일자</option>
                            <option value="popedtime">종료일자</option>
            ...
            <div class="tab-group mt10" >
                <ul v-if="isRead"><!-- 활성화탭 li에 class="active" 추가 -->
                    <li :class="searchData.isdispmagazine === 'T' ? 'active' : ''"><a @click="changeDispMagazine('T')">매거진 메인</a></li>
                    <li :class="searchData.isdispmagazine === 'F' ? 'active' : ''"><a @click="changeDispMagazine('F')">쇼핑메인</a></li>
                </ul>
            </div> 
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                    ...
                    <tr>
                        ...
                        <th>시작일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.popsttime"
                                    class="sort"
                                    :class="[{up : sortData.popsttime === 'popsttime_asc'}, {down : sortData.popsttime === 'popsttime_desc'}]"
                                    @click="sortToggle(sortData.popsttime)">
                            </button>
                        </th>
                        <th>종료일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.popedtime"
                                    class="sort"
                                    :class="[{up : sortData.popedtime === 'popedtime_asc'}, {down : sortData.popedtime === 'popedtime_desc'}]"
                                    @click="sortToggle(sortData.popedtime)">
                            </button>
                        </th>
                        <th>등록일자
                            <button type="button" v-if="isRead" 
                ...
                <tbody v-if="this.list.length > 0">
                    <tr v-for="(row, index) in this.list" :key="index">
                        <td><input type="checkbox" v-model="moveData.targetIdx" :id="row.popidx" :value="index" @change="checkList($event.target.checked)"/></td>
                        <td>{{ $util.isNull(row.sortnum) ? '-' : row.sortnum }}</td>
                        <td class="left"><a class="link" @click="goDetail(row.popidx)">{{ row.subject }}</a></td>
                        <td>
...
import MainPopupRegist from "./MainPopupRegist";
import MainPopupDetail from "./MainPopupDetail";
export default {
    ...
    data() {
        return {
            ...
            sortData: {
                popsttime: 'popsttime_desc',              // 시작일자 정렬기준
                popedtime: 'popedtime_desc',                // 시작일자 정렬기준
    ...
    methods: {
        ...
        searchList(page) {
            ...
            this.$http.post("/admin/operation/setting/mainpopup/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
        ...
        showChange(show) {
            ...
            this.moveData.targetIdx.forEach(function(item){
                list.push(thisObj.list[item].popidx);
            });

            let params = {
                idxlist : list,
                isdisplay : show,
                isdispmagazine: this.searchData.isdispmagazine,
                isloading: false
            }
            ...
            if(show === 'F') { 
                ...
                this.$http.post("/admin/operation/setting/mainpopup/check", params)
                .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/setting/mainpopup/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        fnExcelDownload: function() {
            ...
            this.$http.post('/admin/operation/setting/mainpopup/excel', this.searchData, config)
            .then(result => {
        ...
        // 노출위치 조회 변경
        changeDispMagazine(disp) {
            this.searchData.isdispmagazine = disp;
            this.searchList();
        },
        goRegist() {
