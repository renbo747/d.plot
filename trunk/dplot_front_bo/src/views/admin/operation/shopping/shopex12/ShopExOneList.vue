        ...
        <div class="inner">
            <div class="boxing search-area">
                ...
                <dl>
                    ...
                    <dd>
                        <div class="radio_wrap wide3">
                            <input type="radio" name="group06" v-model="searchData.isshopmain" id="group61" value="" checked><label for="group61">전체</label>
                            <input type="radio" name="group06" v-model="searchData.isshopmain" id="group62" value="T"><label for="group62">노출</label>
                            <input type="radio" name="group06" v-model="searchData.isshopmain" id="group63" value="F"><label for="group63">비노출</label>
                        </div>
                    </dd>
                </dl>                
            </div>
            ...
            <div class="caption-group mt10 clearfix">
                ...
                <div class="btn-group fr">
                    <button type="button" v-if="isWrite" class="btn blue-line" @click="change('S','T')">노출</button>
                    <button type="button" v-if="isWrite" class="btn red-line" @click="change('S','F')">미노출</button>
                    <button type="button" v-if="isWrite" class="btn blue-line" @click="change('M','T')">쇼핑메인 지정</button>
                    <button type="button" v-if="isWrite" class="btn red-line" @click="change('M','F')">쇼핑메인 해지</button>
            ...
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                ...
                <tbody v-else>
                    <tr><td colspan="11">조회 결과가 존재하지 않습니다.</td></tr>
                </tbody>
            </table>
...
export default {
    ...
    methods: {
        ...
        searchList() {
            ...
            this.$http.post("/admin/operation/shopping/shopexone/search", param)
            .then(result => {
        ...
        change(type,show) {
            ...
            if(type === 'S') {
                params.isdisplay = show;
            } else {
                params.isshopmain = show;
            }

            this.$http.post("/admin/operation/shopping/shopexone/check", params)
            .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/shopping/shopexone/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        fnExcelDownload: function() {
            ...
            this.$http.post('/admin/operation/shopping/shopexone/excel', this.searchData, config)
            .then(result => {
                this.$util.debug(result);
            })
            .catch(error => {
        ...
        openGoodsAdditionPopup(index) {
            ...
            this.$eventBus.$emit('modalShow', CommonAddGoodsPopup, param,
                (result) => {
                    ...
                    this.$http.post("/admin/operation/shopping/shopexone/save", param)
                    .then(result => {
                        if (result.statusCode === 200) {
                            alert("저장이 완료되었습니다.");
                            this.searchList();
