<template>
    <div class="content m-leftmenu">
        ...
        <ShopExThreeRegist v-if="showRegist" v-bind:commonCodeList="commonCode" v-on:closeRegist="closeRegist"></ShopExThreeRegist>
        <ShopExThreeDetail v-if="showDetail" :commonCodeList="commonCode" :idx="idx" v-on:closeDetail="closeDetail"></ShopExThreeDetail>
        <div class="inner">
            <div class="boxing search-area">
                ...
                <dl>
                    ...
                    <dd>
                        <div class="radio_wrap wide3">
                            <input type="radio" name="group06" id="group61" v-model="searchData.isintro" value="" checked><label for="group61">전체</label>
                            <input type="radio" name="group06" id="group62" v-model="searchData.isintro" value="T"><label for="group62">제공</label>
                            <input type="radio" name="group06" id="group63" v-model="searchData.isintro" value="F"><label for="group63">제공안함</label>
                        </div>
                    </dd>
                </dl>
                <dl>
            ...
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                ...
                <thead>
                        <tr><th><input type="checkbox" id="chkall" v-model="isallchk" @change="checkAllList($event.target.checked)"/></th>
                        <th>노출순서</th>
                        <th>제목</th>
                        <th>PC 이미지</th>
                        <th>모바일 이미지</th>
                ...
                <tbody v-if="this.list.length > 0">
                    <tr v-for="(row, index) in this.list" :key="index">
                        <td><input type="checkbox" v-model="moveData.targetIdx" :id="row.ex3idx" :value="index" @change="checkList($event.target.checked)"/></td>
                        <td>{{ $util.isNull(row.sortnum) ? '-' : row.sortnum }}</td>
                        <td class="left"><a class="link" @click="goDetail(row.ex3idx)">{{ row.subject }}</a></td>
                        <td>
            ...
            <div class="bottom-group">
                ...
                />
                <div class="btn-group">
                    <button type="button" class="btn blue" v-if="isWrite" @click="goRegist">컨텐츠 등록</button>
                </div>
            </div>
        </div>
    </div>
...
import ShopExThreeRegist from "./ShopExThreeRegist";
import ShopExThreeDetail from "./ShopExThreeDetail";
export default {
    ...
    methods: {
        ...
        searchList(page) {
            ...
            this.$http.post("/admin/operation/shopping/shopexthree/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
        ...
        showChange(show) {
            ...
            this.moveData.targetIdx.forEach(function(item){
                list.push(thisObj.list[item].ex3idx);
            });
            ...
            if(show === 'F') { 
                ...
                this.$http.post("/admin/operation/shopping/shopexthree/check", params)
                .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/shopping/shopexthree/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        fnExcelDownload: function() {
            ...
            this.$http.post('/admin/operation/shopping/shopexthree/excel', this.searchData, config)
            .then(result => {
