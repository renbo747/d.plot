            ...
            <div class="pop-body">
                ...
                <div class="boxing">
                    <div class="form-area">
                        ...
                        <dl>
                            <dt>구분명<i class="essential"></i></dt>
                            <dd><input type="text" v-model="info.tagname" style="width: 100%" placeholder="전시영역 컨텐츠 태그" /></dd>
                        </dl>
                        <dl>
                ...
                <div class="form-area">
                    <table cellpadding="0" cellspacing="0" class="gray-tb">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th>배너 이미지 제공여부<i class="essential"></i></th>
                                <td>
                                    <div class="radio_wrap">
                                        <input type="radio" name="group05" v-model="info.isbnimg" id="group51" value="T" checked=""><label for="group51">제공함</label>
                                        <input type="radio" name="group05" v-model="info.isbnimg" id="group52" value="F"><label for="group52">제공안함</label>
                                    </div>
                                </td>
                            </tr>
                            <tr v-show="info.isbnimg ==='T'">
                                <th>링크(PC)</th>
                                <td>
                            ...
                            <tr v-show="info.isbnimg ==='T'">
                                ...
                                <td>
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 288px; height: 324px;" :class="{'no-image': $util.isNull(files['pcimgfile'])}"><!-- 이미지 없는 경우 no-image 클래스 추가 -->
                                            <img style="width: 288px; height: 324px;" :src="imgPreview['pcimgfile']" alt="배너 이미지(pc)" v-if="!$util.isNull(files['pcimgfile'])">
                                        </div>
                                ...
                                <td>
                                    ...
                                    <div class="scroll-y" style="width: 100%; max-height: 350px; margin-bottom: 0;">
                                        <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                                            ...
                                            <tbody v-else>
                                                <tr><td colspan="7">노출대상 상품이 존재하지 않습니다.</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
...
export default {
    name: 'admin.operation.shopping.shopexfour.detail',
    props: {
    ...
    mounted() {
        this.$http.post('/admin/common/pageAuth/check', {url : this.$options.name}).then(result => {
            ...
            if(this.isRead){
                ...
                this.$http.post("/admin/operation/shopping/shopexfour/detail", param)
                    .then(result =>{
                        this.info = result.data.info;
                        if(this.info.isbnimg === 'T') {
    ...
    methods: {
        ...
        goSave() {
            if(this.checkValidation()) {
                ...
                if(params.isbnimg === 'F') {
                    params.pclinkurl = '';
                    params.ispcnwindow = 'F';
                    params.molinkurl = '';
                    params.ismonwindow = 'F';

                    let deletefile = [];
                    this.deletefile.forEach(obj => {
                        deletefile.push(obj);
                    });
                    if(Object.prototype.hasOwnProperty.call(this.files['pcimgfile'],'idx')){
                        deletefile.push(this.files['pcimgfile'].idx);
                    }                    
                    if(Object.prototype.hasOwnProperty.call(this.files['mobileimgfile'],'idx')){
                        deletefile.push(this.files['mobileimgfile'].idx);
                    }   
                    params.deletefile = new Set(deletefile);                 
                } else {
                ...
                this.$http.post("/admin/operation/shopping/shopexfour/check", param)
                .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)){
                this.$http.post("/admin/operation/shopping/shopexfour/modify", params)
                .then(result => {
                    if(result.statusCode === 200){
                        alert("저장되었습니다.");
                        this.onClose(true);
        ...
        checkValidation() {
            let msg = '';
            let valid = [
                {field: 'info.tagname', type: 'I', name: '구분명', required: true},
                {field: 'info.subject', type: 'I', name: '제목', required: true},
                {field: 'info.desc', type: 'I', name: '설명', required: true},
