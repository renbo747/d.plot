<template>
    <div class="content m-leftmenu">
        ...
        <KeywordRegist v-if="showRegist" v-bind:commonCodeList="commonCode" v-on:closeRegist="closeRegist"></KeywordRegist>
        <KeywordDetail v-if="showDetail" :commonCodeList="commonCode" :idx="idx" v-on:closeDetail="closeDetail"></KeywordDetail>
        <div class="inner">
                ...
                <dl>
                    ...
                    <dd>
                        <select v-model="searchData.stype">
                            <option value="">전체</option>
                            <option value="kwsttime">시작일자</option>
                            <option value="kwedtime">종료일자</option>
            ...
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                    ...
                    <tr>
                        ...
                        <th>시작일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.kwsttime"
                                    class="sort"
                                    :class="[{up : sortData.kwsttime === 'kwsttime_asc'}, {down : sortData.kwsttime === 'kwsttime_desc'}]"
                                    @click="sortToggle(sortData.kwsttime)">
                            </button>
                        </th>
                        <th>종료일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.kwedtime"
                                    class="sort"
                                    :class="[{up : sortData.kwedtime === 'kwedtime_asc'}, {down : sortData.kwedtime === 'kwedtime_desc'}]"
                                    @click="sortToggle(sortData.kwedtime)">
                            </button>
                        </th>
                        <th>등록일자
                            <button type="button" v-if="isRead" 
                ...
                <tbody v-if="this.list.length > 0">
                    <tr v-for="(row, index) in this.list" :key="index">
                        <td><input type="checkbox" v-model="checkedList" :id="row.mzwdidx" :value="row.mzwdidx" @change="checkList($event.target.checked)"/></td>
                        <td>{{ row.no }}</td>
                        <td class="left"><a class="link" @click="goDetail(row.mzwdidx)">{{ row.subject }}</a></td>
                        <td>
...
import KeywordRegist from './KeywordRegist.vue';
...
export default {
    ...
    data() {
        return {
            ...
            sortData: {
                kwsttime: 'kwsttime_desc',              // 시작일자 정렬기준
                kwedtime: 'kwedtime_desc',                // 시작일자 정렬기준
    ...
    methods: {
        ...
        searchList(page) {
            ...
            this.$http.post("/admin/operation/magazine/keyword/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
        ...
        showChange(show) {
            ...
            this.$http.post("/admin/operation/magazine/keyword/check", params)
            .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/magazine/keyword/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        checkAllList: function(value) {
            ...
            if (value) {
                this.list.forEach(item => {
                    this.checkedList.push(item.mzwdidx);
                });
            }
        },
        // 목록 개별체크
        ...
        fnExcelDownload: function() {
            ...
            this.$http.post('/admin/operation/magazine/keyword/excel', this.searchData, config)
            .then(result => {
