...
export default {
    ...
    methods: {
        ...
        updateReply(index, index2, commentidx) {
            this.$util.debug("updateReply .......");
            this.$util.debug(index);
            this.$util.debug(index2);
            this.$util.debug(commentidx);
            this.$util.debug(this.reviseRlyList[index2]);
        ...
        infiniteHandler($state) {
            if (this.isloading) {
                ...
                this.$http.post('/shop/commentlist', param).then(result => {
                    if (result.statusCode == 200) {
                        if (result.data.cmtlist.length == 0) {
                            $state.complete();
                            ...
                            this.isloading = true;
                            this.currentPage += 1;
                            this.commentList = [...this.commentList || [], ...result.data.cmtlist];
                        }
                        this.$util.debug(this.commentList);
                    }
                });
            }
        },
        ...
        snsShareModal() {
            let description = "";
            description += "기간: " +  this.Detail.startdate + "~" +  this.Detail.enddate;
            let param = {
                kakao: {
                    objectType: "feed",
                    content: {
