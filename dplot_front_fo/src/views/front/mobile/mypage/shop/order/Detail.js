...
export default {
    ...
    methods: {
        getOrderDetail() {
            ...
            this.$http.post('/mypage/order/detail', param).then(result => {
                if (result.statusCode == 200) {
                    ...
                    for(let i = 0 ; i < list.length ; i++) {
                        ...
                        //옵션명 설정
                        let optionnames = list[i].frontoptionname.split('|');
                        list[i].opthtml = '';
                        ...
                        this.paypoint = this.paypoint + list[i].paypoint;
        ...
        selectAddr(addr) {
            ...
            this.$eventBus.$emit('confirm','배송지변경','배송지를 변경하시겠습니까?', ()=>{
                ...
                this.orderInfo.isdefault = addr.isdefault;
                ...
                this.$http.post('/mypage/order/rcvsave', this.orderInfo).then(result => {
                    if (result.statusCode == 200) {
                        this.$toast.clear();
        ...
        allCancel() {
            ...
            this.$router.push({name:'mypage-claim-cancel-apply', query : {items :  this.$front.makeClaimStr(orderList)}});
        },
        goReceipt() {
