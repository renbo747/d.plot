    ...
    methods: {
        ...
        setClaimItem(item) {
            ...
            if(!this.$util.isBlank(item.bfoptionname)){
                ...
                let bfitem = this.$util.deepClone(item);
                ...
                if(optionnames.length > 0) {
                    ...
                    bfitem.opthtml = optionnames.join('<span class="dp-bar h10"></span>');  
                }
                item = bfitem;
        ...
        handleFooterButton(item, val) {
            switch (val) {
                ...
                case '교환배송조회':
                    ...
                    break;
                case 'AS전용 문의' : 
                    this.$router.push({name : 'apply-as', params:{pid:item.ordgdidx}});
                    break;
                case '주문취소' :
                    item.clmcnt = 1;
                    this.$router.push({name:'mypage-claim-cancel-apply', query : {items :  this.$front.makeClaimStr([item])}});
                    break;
                case '수취확인' :
                    item.ispbgoods = 'T'
                    this.$eventBus.$emit('confirm', '수취확인', '수취확인 처리하시겠습니까?', ()=>{
                        this.$http.post('/mypage/order/delivcomplete', item).then(result => {
                ...
                case '반품신청' :
                    item.clmcnt = 1;
                    this.$router.push({name:'mypage-claim-return-apply', query : {items :  this.$front.makeClaimStr([item])}});
                    break;
                case '교환신청' :
                    item.clmcnt = 1;
                    this.$router.push({name:'mypage-claim-exchange-apply', query : {items :  this.$front.makeClaimStr([item])}});
                    break;
                case '구매확정':
                    if(item.clmingcnt > 0) {
                    ...
                    param.ordcnt = param.ordcnt - param.cncclmcnt;
                ...
                case '교환철회' :
                    ...
                    this.$eventBus.$emit('confirm', val, msg +'을 철회하시겠습니까?',()=>{
                        ...
                        this.$http.post('/mypage/claim/cancel', param).then(result => {
                            if (result.statusCode == 200) {
                                this.$toast.clear();
                                this.$toast.open(msg + "이 철회되었습니다.");
                                this.$router.replace({name:'mypage-claim', params:{init:true}});
                            }
                        });
                    });
                    break;
                case '리뷰작성' :
                    this.$router.push({name:'mypage-my-review-write' , query:{orderidx:item.orderidx, ordgdidx:item.ordgdidx}});
                    break;
                default:
                    break;
            }
