import AddressList from '@views.mobile/order/popup/AddressList.vue'
...
export default {
    ...
    methods: {
      getClaimList(claimList) {
        ...
        this.$http.post('/mypage/claim/apply', param).then(result => {
          if (result.statusCode == 200) {
            ...
            this.dispitems = list;
            this._setGiftList(result.data.giftlist);
          }
        });
      },
      //사은품 모델변경
      _setGiftList(list) {
        ...
        for(let i = 0 ; i < list.length ; i++) {
            ...
            if(list[i].isadd == 'F') {
              //수량변경 사은품의 경우
              this.selectGiftList.push(list[i]);
              continue;
            }
            list[i].chk = false;
            //사은품구조 변경
      ...
      //배송지변경 팝업
      changeAddr(type){
        this.$eventBus.$emit('showModal', AddressList, 'addressListModal', {}, (addr) => {
      ...
      saveClaimApply() {
        ...
        if(this.ordstatus == 'ODS001'){
          this.$http.post('/mypage/order/cancel', this.bforder).then(result => {
            if (result.statusCode == 200) {
              this.$router.replace({name:'mypage-order', params:{init:true}});
            }
          });
          return;
        }
          ...
          for(let j = 0 ; j < this.giftList[i].detail.length ; j++) {
              for(let y = 0 ; y < this.giftList[i].detail[j].length ; y++) {
                  if(this.giftList[i].detail[j][y].chk) {
                    temp.push(this.giftList[i].detail[j][y]);
                  }
              }
          }
        }
        ...
        let param = {
          ...
          giftlist : this.selectGiftList.concat(temp)
        }
        //크레임별 사유 및 상태
        ...
        this.$http.post('/mypage/claim/save', param).then(result => {
          if (result.statusCode == 200) {
