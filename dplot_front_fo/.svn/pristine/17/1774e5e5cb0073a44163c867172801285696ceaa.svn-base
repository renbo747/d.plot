          ...
          <div class="inquiry-detail-state-header d-flex">
            ...
            <div class="inquiry-detail-state-header__right">
              <div v-if="data.isreply ==='답변대기'" class="inquiry-detail-edit text-gray-800">
                <span class="inquiry-detail-edit-text text-gray-800 font-weight-400 cursor" @click="inquiryDelete">삭제</span
                >
                <span class="dp-bar"></span>
                <span class="inquiry-detail-edit-text text-gray-800 font-weight-400 cursor" @click="goWrite">수정</span>
              </div>
            </div>
          </div>
          <div class="inquiry-detail-state-body">
            <header class="detail-state-body-header">
              <h2 class="detail-state-body-header__text dp-p text-gray-700 font-weight-400 mb-0" v-if="isInquiry === 'Inquiry'">
                [{{ data.inquirytype }}] 
              </h2>
              <h2 class="detail-state-body-header__text dp-p text-gray-700 font-weight-400 mb-0" v-else>
                [{{ data.qnatypename }}]
              </h2>
            </header>
          ...
          <div class="detail-product__info" v-if="isInquiry === 'Inquiry'">
            <div class="detail-product__info__item d-flex mb-10">
              <div class="detail-product__info__item__title">
                <p class="mb-0 info__item__title__p font-weight-400 text-gray-700">
                  <!--주문번호-->
                </p>
              </div>
                ...
                <div class="inquiry-qna-body__thumbnail d-flex">
                  ...
                  <div class="inquiry-qna-body__thumbnail__info">
                    ...
                    <div class="d-flex align-items-center">
                      <span v-if="!$util.isNull(data.optionnm1)"
                        class="qna__body-text font-weight-400 text-gray-700"
                      >
                        옵션 : {{data.optionnm1}}
                      </span>
                      <span v-if="!$util.isNull(data.optionnm2)" class="dp-bar"></span>
                      <span v-if="!$util.isNull(data.optionnm2)" class="qna__body-text font-weight-400 text-gray-700"
                        >{{data.optionnm2}}</span
                      >
                    </div>
                  </div>
          ...
          <div class="detail__info__text-box not-empty dp-panel" v-else>
            <div class="text-box-flex d-flex">
              <span class="detail__info__text text-black font-weight-400 mb-20">
                <div v-html="$util.toEscape(data.repcontent)" />
              </span>
              <span class="text-gray-700 font-weight-400 dp-p-sm"
                >{{data.repregdate}}</span
              >
            </div>
          </div>
