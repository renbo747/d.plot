<template>
  <div class="tab-area" style="height: calc(90vh - 246px);">
    <div class="clearfix">
      <div class="bar-title small fl">D포인트 적립/차감/소멸내역</div>
    </div>
    <div class="mini-dash col5">
      <dl>
        <dt>총 적립</dt>
        <dd><p>{{ pay }}p</p></dd>
      </dl>
      ...
      <dl>
        <dt>소멸예정</dt>
        <dd><p>{{ expected }}p<span>익일 소멸예정:{{ tomorrow }}</span></p></dd>
      </dl>
    </div>
    ...
    <table cellpadding="0" cellspacing="0" class="data-tb align-c">
      ...
      <tbody v-if="epointList.length > 0">
      <tr v-for="(item, index) in epointList" :key="index">
    ...
    <div class="bottom-group">
      ...
      <div class="btn-group">
        <button type="button" class="btn blue" @click="goNewPost">D포인트 수동지급</button>
      </div>
    </div>
    <OrderDetail v-if="isOrderDetailShow" :activeOrderCode="activeOrderCode" @closeDetail="closeOrderDetailPopup"></OrderDetail>
    <EpointNewPost v-if="isNewPostShow" ref="refNewPost" :page-code="pageCode" @close="closeNewPostPopup" />
  </div>
</template>
...
export default {
  name: "AdminMemberEPoint",
  props: ['activeUserNo'],
  components: { CommonDatePicker, CommonPageNavigator, OrderDetail, EpointNewPost },
  data() {
    return {
      epointList: [],
      pageCode: {
  ...
  methods : {
    onSearch() {
      ...
      this.$http.post('/admin/member/epoint/list', params).then(result => {
        this.epointList = result.data.list;
    ...
    downloadExcel() {
      ...
      this.$http.post('/admin/member/epoint/exceldown', this.searchData, config)
          .then(result => {
