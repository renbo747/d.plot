    ...
    <div class="tab-area" :style="isPartner? 'height: calc(90vh - 100px);':'height: calc(90vh - 290px);'">
        <template v-for="(item, index) in claimList">
            ...
            <div class="clearfix" :key="'title'+index">
                <div class="bar-title small fl">취소상품 [취소 {{ index + 1 }}차]{{ $util.isNull(item.clmstatusname)? '' : ' - '+item.clmstatusname }}</div>
            </div>
            <table cellpadding="0" cellspacing="0" class="data-tb align-c mt0" :key="'list'+index">
                <caption>취소상품 목록</caption>
                <colgroup>
                    <col width="5.8%" /><!-- 취소 신청일 -->
                    <col width="5.8%" /><!-- 취소 완료일 -->
                    <col width="4.5%" /><!-- 판매구분 -->
                    <col width="5.5%" /><!-- 파트너사 -->
                    <col width="6.5%" /><!-- 상품코드 -->
                    <col width="3.5%" /><!-- 단품코드 -->
                ...
                <tbody v-if="item.claimgoodslist.length > 0">
                    <tr v-for="row in item.claimgoodslist" :key="row.clmgdidx">
                        ...
                        <td><a href="javascript:void(0);" class="link" @click="goCancelDetail(row)">{{ row.clmno }}</a></td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="left">
                ...
                <div class="right">
                    <template v-if="Number(item.rtndadadelivamt) + Number(item.rtnptndelivamt) > 0">
                        ...
                        <table cellpadding="0" cellspacing="0" class="gray-tb lower">
                            ...
                            <tbody>
                                <tr v-if="!isPartner">
                                    <th>자사배송비</th>
                                    <td class="right">{{ $util.maskComma(item.rtndadadelivamt) }}</td>
                                </tr>
                                <tr>
                                    <th>파트너사 배송비</th>
                                    <td class="right">{{ $util.maskComma(item.rtnptndelivamt) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </template>
                    ...
                    <table cellpadding="0" cellspacing="0" class="gray-tb lower">
                        ...
                        <tbody>
                            <tr>
                                <th>결제예정금액</th>
                                <td class="right">{{ $util.maskComma(item.addrpaytotprice) }}</td>
                            </tr>
                        </tbody>
                    </table>
                    <template v-if="Number(item.addrpaytotprice) > 0">
                        <div class="bar-title small">추가결제내역<span class="normal txt-orange ml10" v-if="item.cncstatus === $store.getters['ADMIN'].CNC_WAITING_PAYMENT">추가금 발생일로부터 {{ $util.maskComma(item.elapsedaycnt) }}일 경과</span></div>
                        <table cellpadding="0" cellspacing="0" class="gray-tb lower">
                            <colgroup>
                                <col width="150px">
                                <col width="">
                    ...
                    <div class="bar-title small">환불금액</div>
                    <table cellpadding="0" cellspacing="0" class="gray-tb lower">
                        <colgroup>
                            <col width="150px">
                            <col width="">
                        ...
                        <tbody>
                            <tr>
                                <th>환불예정금액</th>
                                <td class="right"><strong class="large-txt">{{ $util.maskComma(item.rtnamt) }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <template v-if="$util.isNull(claimList) || claimList.length === 0">
            <div class="clearfix">
                <div class="bar-title small fl">취소상품</div>
            </div>
            <table cellpadding="0" cellspacing="0" class="data-tb align-c mt0">
                ...
                <tbody>
                    <tr><td colspan="16">취소내역이 존재하지 않습니다.</td></tr>
                </tbody>
            </table>
        </template>
...
export default {
    ...
    components:{
        GoodsDetail, CancelAdminDetail, CancelPartnerDetail
    },
    data() {
        return {
            isPartner: false,
            ...
            isShowCancelAdminDetail: false,     // 클레임 관리자 취소상세 팝업 노출여부
            isShowCancelPartnerDetail: false,   // 클레임 파트너 취소상세 팝업 노출여부
        }
    },
    mounted() {
        ...
        this.getOrderCncInfo();
    },
    methods: {
        // 취소내역 조회
        getOrderCncInfo: function() {
            let params = {
