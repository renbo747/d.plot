...
export default {
    name: 'admin.goods.manage.goodsDetail',
    props: ['activeGoodsNo'],
    components: {
        CommonNavigator,
        CommonEditor,
    ...
    data() {
        return {
            ...
            // Anchor 탭
            tabObject: {
                goodsTab1: {href: '#goodsTab1', tabName: '기본정보', isActive: false, isOpen: true},
                goodsTab2: {href: '#goodsTab2', tabName: '판매정보', isActive: false, isOpen: true},
                goodsTab3: {href: '#goodsTab3', tabName: '옵션정보', isActive: false, isOpen: true},
                goodsTab4: {href: '#goodsTab4', tabName: '이미지정보', isActive: false, isOpen: true},
                goodsTab5: {href: '#goodsTab5', tabName: '배송정보', isActive: false, isOpen: true},
                goodsTab6: {href: '#goodsTab6', tabName: '상품정보고시', isActive: false, isOpen: true},
                goodsTab7: {href: '#goodsTab7', tabName: '추가정보', isActive: false, isOpen: true}
            },
            commonCode: {
                muappchtype: [],    // 적용채널
            ...
            editauth: {
                ...
                goodsname: {isauth: true},
                summary: {isauth: true},
                image: {isauth: true},
                price: {isauth: true},
                marketprice: {isauth: true},
                commrate: {isauth: true},
                delividx: {isauth: true},
                stockcnt: {isauth: true},
                soldout: {isauth: true},
                display: {isauth: true},
                option: {isauth: true},
                notify: {isauth: true},
                pccontent: {isauth: true},
                mocontent: {isauth: true}
            },
            // 기본정보
            basicInfo: {
            ...
            sellInfo: {
                istaxfree: '',      //면세여부
                orggoodsselltype: '',  //기존상품판매상태
                goodsselltype: '',  //상품판매상태
                goodsselltypename: '',  //상품판매상태명
                disstday: '',       //전시시작일
                disstdate: '',      //전시시작일자
                disedday: '',       //전시종료일
                diseddate: '',      //전시종료일자
            ...
            // 이미지정보
            imageInfo: {
                issamepcimg: false, //PC이미지동일여부
                sameimgidx: '',     //PC이미지일련번호
                issamepccont: false,//PC상세내용동일여부
                isusenotice: 'F',   //공지사용여부
                isuseintro: 'F',    //인트로사용여부
                noticecontent: '',  //공지내용
                introcontent: '',   //인트로내용
                pccontent: '',      //PC상세설명
                mobilecontent: '',  //모바일상세설명
    ...
    methods : {
        ...
        onInit:function() {
            ...
            // 상품번호 세팅
            this.basicInfo.goodsno = this.activeGoodsNo;
            // 유저정보 세팅
        ...
        getCommonCodeList: function() {
            let cmclassArr = ['MUAPPCHTYPE', 'GOODSSELLTYPE', 'GOODSDIVTYPE', 'DADAMEMBERTYPE', 'MEMLVTYPE', 'KCDIVTYPE', 'COLORTYPE', 'MUADDAUTHTYPE', 'GICONTYPE'];
            ...
            this.$http.post('/admin/goods/regist/goods/detail', param)
                .then(result => {
                    ...
                    if (result.statusCode == '200') {
                        ...
                        this.basicInfo.orggoodsname = this.basicInfo.goodsname;
                        this.basicInfo.orggoodsapprtype = this.basicInfo.goodsapprtype;
                        this.basicInfo.orgispbgoods  = this.basicInfo.ispbgoods;
                        if (this.isPartner) {
                            // 파트너사 수수료율 초기화
                        ...
                        // 파트너사 상품상세 수정권한 세팅
                        this.commonCode.muaddauthtype.forEach(item=> {
                            let addItem = {};
                            addItem.authKey = item.detail;
                            addItem.authName = item.codename;
                            addItem.authCode = item.cmcode;
                            addItem.isauth = true;
                            if (this.isPartner && this.basicInfo.goodsapprtype===this.$store.getters['ADMIN'].GOODS_STATUS_APPROVAL && data.editauth.indexOf(item.cmcode) < 0) {
                                addItem.isauth = false;
                            }
                            this.$set(this.editauth, item.detail, addItem);
                        });

                        for(const [key] of Object.entries(data.sellinfo)) {
                            let paramArr = ['disstdate', 'dissthour', 'disstmin', 'diseddate', 'disedhour', 'disedmin'];
                            if (paramArr.indexOf(key) > -1) {
                                ...
                                this.sellInfo[key] = data.sellinfo[key];
                                if (key == 'mumemlvtype' && !this.$util.isNull(this.sellInfo[key])) {
                                    this.sellInfo.mumemlvtypeArr = this.sellInfo[key].split(',');
                                }
                            }
                        }
                        this.sellInfo.orggoodsselltype = this.sellInfo.goodsselltype;
                        for(const [key] of Object.entries(data.imageinfo)) {
                            this.imageInfo[key] = data.imageinfo[key];
                            if (key == 'mugicontype' && !this.$util.isNull(this.imageInfo[key])) {
                                this.imageInfo.mugicontypeArr = this.imageInfo[key].split(',');
                            }
                        }
                        ...
                        for(const [key] of Object.entries(data.etcinfo)) {
                            ...
                            if (key == 'mumembertype' && !this.$util.isNull(this.etcInfo[key])) {
                        ...
                        setTimeout(function () {
                            ...
                            if(!this.isWrite){
                                let buttons = this.$el.getElementsByTagName('button');
                                for(let button of buttons){
                                    if(button.className !== 'btn-search' && button.className !== 'pop-close') {
        ...
        getCateTemplateList: function() {
            this.$http.post('/admin/goods/regist/catetemp/list', {isloding: false})
                .then(result => {
                    this.$util.debug(result);
        ...
        openSearchOrgCodePopup: function(obj) {
            let param = { 
                ...
                linkedList: this.$util.isNull(obj.linkedlist)? []:obj.linkedlist
            };
        ...
        openDelivTempListPopup: function() {
            let param = {dealerno: this.basicInfo.dealerno};
            ...
            // 위탁옵션 일괄등록
                ...
                for (let i=1; i<excelData.length; i++) {
                    ...
                    let additem = {
                        ...
                        ismaingoods: listcnt>0 ? 'F':'T',
                        isoptdisplay: 'T',
                        marketprice: (this.isPartner && !this.editauth.marketprice.ieauth)? '' : item.marketprice,
                        price: (this.isPartner && !this.editauth.price.ieauth)? '' : item.price,
                        commrate: this.isPartner? this.optionInfo.commrate : (this.$util.isNull(item.commrate)? this.optionInfo.commrate : item.commrate),
                        stockcnt: (this.isPartner && !this.editauth.stockcnt.ieauth)? '' : item.stockcnt,
                        colortype: null,    //item.colortype,
                        optionnm1: item.optionnm1,
                        optionnm2: item.optionnm2,
                        optionnm3: item.optionnm3,
        ...
        changeGoodsAppr: function(value) {
            if (value == this.$store.getters['ADMIN'].GOODS_STATUS_APPROVAL) {
                if (confirm('승인 하시겠습니까?')) {
                    let params = {
                        goodsnolist: [this.basicInfo.goodsno],
                        goodsapprtype: value,
                        contents : '승인합니다.'
                    }
            ...
            if (checkResult && this.basicInfo.ispbgoods === 'T') {
                for (let i=0; i<this.optionInfo.optionList.length; i++) {
                    ...
                    for (let j=0; j<this.optionInfo.optionItemList.length; j++) {
                        ...
                        let valName= "param.optionnm"+(j+1);
                        let optionNm = (new Function('param','return '+valName)(item));
        ...
        goSave: function() {
            ...
            if (this.checkSaveVaildation()) {
                ...
                if (confirm('저장 하시겠습니까?')) {
                    ...
                    // 관리자/직매입상품-승인완료
                    if (!this.isPartner && this.basicInfo.ispbgoods === 'T' && this.basicInfo.goodsapprtype != this.$store.getters['ADMIN'].GOODS_STATUS_REQ) {
        ...
        save: function() {
            ...
            if (this.iscopy) {
                params.iscopy = 'T';
                params.copygoodsno = params.goodsno;
                params.goodsno = '';
            }
            // 파일세팅
            let files = [];
            ...
            if (!this.$util.isNull(this.files.pcrepreimgfile)) {
                if (!this.$util.isNull(this.files.pcrepreimgfile.file)) {
                    files.push({key: constants.IMG_TYPE_GOODS_IMG_PC_B, file: this.files.pcrepreimgfile.file});
                } else if (!this.$util.isNull(this.files.pcrepreimgfile.idx)) {
                    copyFileList.push({key: constants.IMG_TYPE_GOODS_IMG_PC_B, file: null, fileidx: this.files.pcrepreimgfile.idx});
                }
            }
            if (!this.$util.isNull(this.files.morepreimgfile)) {
                if (!this.$util.isNull(this.files.morepreimgfile.file)) {
                    files.push({key: constants.IMG_TYPE_GOODS_IMG_MO_B, file: this.files.morepreimgfile.file});
                } else if (!this.$util.isNull(this.files.morepreimgfile.idx)) {
                    copyFileList.push({key: constants.IMG_TYPE_GOODS_IMG_MO_B, file: null, fileidx: this.files.morepreimgfile.idx});
                }
            }
            if (this.files.addimgfilelist.length > 0) {
                for(let i=0; i<this.files.addimgfilelist.length; i++) {
                    if (!this.$util.isNull(this.files.addimgfilelist[i].file)) {
                        files.push({key: constants['IMG_TYPE_GOODS_IMG_ADD_B'+(i+1)], file: this.files.addimgfilelist[i].file});
                    } else if (!this.$util.isNull(this.files.addimgfilelist[i].idx)) {
                        copyFileList.push({key: constants['IMG_TYPE_GOODS_IMG_ADD_B'+(i+1)], file: null, fileidx: this.files.addimgfilelist[i].idx});
                    }
                }
            }
            if (this.files.kcfilelist.length > 0) {
                for(let i=0; i<this.files.kcfilelist.length; i++) {
                    if (!this.$util.isNull(this.files.kcfilelist[i].file)) {
                        files.push({key: constants['IMG_TYPE_GOODS_KC_CERT']+(i+1), file: this.files.kcfilelist[i].file});
                    } else if (!this.$util.isNull(this.files.kcfilelist[i].idx)) {
                        copyFileList.push({key: constants['IMG_TYPE_GOODS_KC_CERT']+(i+1), file: null, fileidx: this.files.kcfilelist[i].idx});
                    }
                }
            }
            params.files = files;
            if (this.iscopy) {
                params.copyfilelist = copyFileList;
            }
