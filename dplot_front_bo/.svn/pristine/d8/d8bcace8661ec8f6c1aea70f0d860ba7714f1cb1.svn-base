...
export default {
    name: 'admin.operation.shopping.shopexfive.detail',
    props: {
    ...
    mounted() {
        this.$http.post('/admin/common/pageAuth/check', {url : this.$options.name}).then(result => {
            ...
            if(this.isRead){
                ...
                this.$http.post("/admin/operation/shopping/shopexfive/detail", param)
                    .then(result =>{
    ...
    methods: {
        ...
        update(params) {
            if(confirm(params.msg)){
                this.$http.post("/admin/operation/shopping/shopexfive/modify", params)
                .then(result => {
                    if(result.statusCode === 200){
                        alert("저장되었습니다.");
                        this.onClose(true);
