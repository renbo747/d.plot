                                ...
                                <td>
                                    ...
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 720px; height: 640px;" :class="{'no-image': $util.isNull(files['smobileimgfile'])}">
                                            <img style="width: 720px; height: 640px;" :src="imgPreview['smobileimgfile']" alt="쇼핑메인 이미지(모바일)" v-if="!$util.isNull(files['smobileimgfile'])">
                                        </div>
                                        <button type="button" class="btn blue-line mt10" style="width: 100%;"
...
export default {
    ...
    name: 'admin.operation.magazine.trend.regist',
    components: {
        CommonDatePickerFromTo,
        CommonEditor
    },
    ...
    methods: {
        setInit() {
            ...
            this.$http.post("/admin/operation/magazine/trend/category", params)
            .then(result => {
            ...
            if(this.checkValidation()) {
                ...
                if(this.info.isdispshop === 'T') {
                    if(!this.$util.isNull(this.files.spcimgfile)){
                        files.push({key: 'spcimgfile', file: this.files.spcimgfile.file});
                    }
                    if(!this.$util.isNull(this.files.smobileimgfile)){
                        files.push({key: 'smobileimgfile', file: this.files.smobileimgfile.file});
                    }
                    params.keywordlist = this.keywordlist;
                }
                params.files= files;

                if(params.isdispshop === 'T' && params.isdisplay === 'T') {
                     let param = {
                        overmsg: '저장하시겠습니까?',
                        zeromsg: '저장',
                        isdisplay: params.isdisplay,
                        isdispshop: params.isdispshop,
        ...
        checkSave(params) {
            if(confirm(params.msg)){
                this.$http.post("/admin/operation/magazine/trend/save", params)
                .then(result => {
                    if(result.statusCode === 200){
                        alert("저장되었습니다.");
                        this.onClose(true);
        ...
        checkValidation() {
            ...
            let valid = [
                ...
                {field: 'info.contents', type: 'I', name: '내용', required: true},
