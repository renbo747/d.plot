<template>
    <div id="modal-wrap" class="modal" style="display: block;">
        <div class="modal-content" style="width: 1400px;">
            <div class="pop-header">
                <h2>트렌드컨텐츠 상세</h2>
                <button type="button" class="pop-close" @click="onClose"></button>
            </div>
            <div class="pop-body">
                <div class="gray-box mg0 clearfix">
                    <div class="fl">
                        <span>작성자 : {{info.reguserid}}</span>
                    </div>
                    <div class="fr txt-gray">
                        <span>등록일 : {{info.regdate}}</span>
                        <span class="left-bar">수정일 : {{info.moddate}}</span>
                    </div>
                </div>
                <div class="bar-title mt10">기본정보</div>
                <div class="boxing">
                    <div class="form-area">
                        <dl>
                            <dt>노출상태<i class="essential"></i></dt>
                            <dd>
                                <div class="radio_wrap wide">
                                    <input type="radio" name="group00regist" id="group01regist" v-model="info.isdisplay" value="T" checked/><label for="group01regist">노출</label>
                                    <input type="radio" name="group00regist" id="group02regist" v-model="info.isdisplay" value="F" /><label for="group02regist">미노출</label>
                                </div>
                            </dd>
                        </dl>
                        <dl>
                            <dt>제목<i class="essential"></i></dt>
                            <dd><input type="text" style="width: 100%" v-model="info.subject" placeholder="트렌드영역 컨텐츠 제목" /></dd>
                        </dl>
                        <dl>
                            <dt>설명</dt>
                            <dd><input type="text" style="width: 100%" v-model="info.desc" placeholder="트렌드영역 컨텐츠 설명" /></dd>
                        </dl>
                    </div>
                </div>
                <div class="bar-title">조건설정</div>
                <div class="form-area">
                    <table cellpadding="0" cellspacing="0" class="gray-tb">
                        <colgroup>
                            <col width="180px">
                            <col width="">
                        </colgroup>
                        <tbody>
                            <tr>
                                <th>전시기간<i class="essential"></i></th>
                                <td>
                                    <CommonDatePickerFromTo
                                        :fromYYYYMMDD="timeInfo.startYYYYMMDD"
                                        :fromHH="timeInfo.startHH"
                                        :fromMM="timeInfo.startMM"
                                        :toYYYYMMDD="timeInfo.toYYYYMMDD"
                                        :toHH="timeInfo.toHH"
                                        :toMM="timeInfo.toMM"
                                        @getDate="getEventTimeDate"
                                    />
                                </td>
                            </tr>
                            <tr>
                                <th>노출채널<i class="essential"></i></th>
                                <td>
                                    <div class="check-wrap">
                                        <input type="checkbox" v-model="checkObj.isallmuappch" id="isallmuappch" true-value="T" false-value="F" @change="checkAllAppchtype">
                                        <label for="isallmuappch">전체</label>
                                    </div>
                                    <div class="check-wrap" v-for="item in commonCode.muappchtype" :key="item.cmcode">
                                        <input type="checkbox" v-model="checkObj.muappchtypearr" :id="'muappchtype_'+item.cmcode" true-value="[]" :value="item.cmcode">
                                        <label :for="'muappchtype_'+item.cmcode">{{item.codename}}</label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>대상회원유형<i class="essential"></i></th>
                                <td>
                                    <div class="check-wrap">
                                        <input type="checkbox" v-model="checkObj.isallmumember" id="isallmumember" true-value="T" false-value="F" @change="checkAllMembertype">
                                        <label for="isallmumember">전체</label>
                                    </div>
                                    <div class="check-wrap" v-for="item in commonCode.mumembertype" :key="item.cmcode">
                                        <input type="checkbox" v-model="checkObj.mumembertypearr" :id="'mumembertype_'+item.cmcode" true-value="[]" :value="item.cmcode">
                                        <label :for="'mumembertype_'+item.cmcode">{{item.codename}}</label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>대상회원등급<i class="essential"></i></th>
                                <td>
                                    <div class="check-wrap">
                                        <input type="checkbox" v-model="checkObj.isallmumemlv" id="isallmumemlv" true-value="T" false-value="F" @change="checkAllMemLvtype">
                                        <label for="isallmumemlv">전체</label>
                                    </div>
                                    <div class="check-wrap" v-for="item in commonCode.mumemlvtype" :key="item.cmcode">
                                        <input type="checkbox" v-model="checkObj.mumemlvtypearr" :id="'mumemlvtype_'+item.cmcode" true-value="[]" :value="item.cmcode">
                                        <label :for="'mumemlvtype_'+item.cmcode">{{item.codename}}</label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>카테고리<i class="essential"></i></th>
                                <td>
                                    <select v-model="info.mzcateidx" style="width: 450px;">
                                        <option value="">매거진 카테고리를 선택하세요.</option>
                                        <option v-for="(row, c) in this.categorylist" :key="c" :value="row.mzcateidx">{{ row.subject }}</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <th>이미지(PC)<i class="essential"></i></th>
                                <td>
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 620px; height: 457px;" :class="{'no-image': $util.isNull(files['pcimgfile'])}"><!-- 이미지 없는 경우 no-image 클래스 추가 -->
                                            <img style="width: 620px; height: 457px;" :src="imgPreview['pcimgfile']" alt="이미지(pc)" v-if="!$util.isNull(files['pcimgfile'])">
                                        </div>
                                        <button type="button" class="btn blue-line mt10" style="width: 100%;"
                                            v-if="$util.isNull(files['pcimgfile'])" @click="fileAttach('pcimgfile')">파일 올리기</button>
                                        <input type="file" ref="pcimgfile" @change="handleFileUpload('pcimgfile')" accept="image/jpeg, image/png" hidden/>
                                        <button type="button" class="btn blue-line mt10" style="width: calc(50% - 3px);"
                                            v-if="!$util.isNull(files['pcimgfile'])" @click="fileAttach('pcimgfile')">변경</button>
                                        <button type="button" class="btn red-line mt10" style="width: calc(50% - 4px);"
                                            v-if="!$util.isNull(files['pcimgfile'])" @click="removeFile('pcimgfile')">삭제</button>
                                    </div>
                                    <div class="img-with-text text">
                                        <p class="txt-orange"><i class="icon-alert"></i>대표 이미지입니다. 보기 쉬운 간결한 이미지를 활용해 주세요.</p>
                                        <p class="txt-orange"><i class="icon-alert"></i>사이즈: 620*457 / 용량: 10MB 이하 / 파일 : JPG, JPEG, PNG</p>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>이미지(모바일)<i class="essential"></i></th>
                                <td>
                                    <div class="mb10">
                                        <input type="checkbox" v-model="info.copyimgcheck" @change="setSameAsPcepreImg" id="copy-img"><label for="copy-img">PC 대표 이미지를 복사</label>
                                    </div>
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 720px; height: 640px;" :class="{'no-image': $util.isNull(files['mobileimgfile'])}">
                                            <img style="width: 720px; height: 640px;" :src="imgPreview['mobileimgfile']" alt="이미지(모바일)" v-if="!$util.isNull(files['mobileimgfile'])">
                                        </div>
                                        <button type="button" class="btn blue-line mt10" style="width: 100%;"
                                            v-if="$util.isNull(files['mobileimgfile'])" @click="fileAttach('mobileimgfile')">파일 올리기</button>
                                        <input type="file" ref="mobileimgfile" @change="handleFileUpload('mobileimgfile')" accept="image/jpeg, image/png" hidden/>
                                        <button type="button" class="btn blue-line mt10" style="width: calc(50% - 3px);"
                                            v-if="!$util.isNull(files['mobileimgfile'])" @click="fileAttach('mobileimgfile')">변경</button>
                                        <button type="button" class="btn red-line mt10" style="width: calc(50% - 4px);"
                                            v-if="!$util.isNull(files['mobileimgfile'])" @click="removeFile('mobileimgfile')">삭제</button>
                                    </div>
                                    <div class="img-with-text text">
                                        <p class="txt-orange"><i class="icon-alert"></i>모바일 리스팅 및 와이드형 화면에 노출되는 이미지를 업로드 해 주세요.</p>
                                        <p class="txt-orange"><i class="icon-alert"></i>사이즈: 720*640 / 용량: 10MB 이하 / 파일 : JPG, JPEG, PNG</p>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>내용(PC)<i class="essential"></i></th>
                                <td>
                                    <div>
                                        <CommonEditor ref="editor"/>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>내용(모바일)<i class="essential"></i></th>
                                <td>
                                    <div class="mb10">
                                        <button type="button" class="btn blue-line" @click="copyByContent">PC 내용을 복사</button>
                                    </div>
                                    <div class="mt10">
                                        <CommonEditor ref="moeditor"/>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <th>쇼핑메인<i class="essential"></i></th>
                                <td>
                                    <div class="radio_wrap wide">
                                        <input type="radio" name="group04" v-model="info.isdispshop" id="group41" value="T" checked=""><label for="group41">노출</label>
                                        <input type="radio" name="group04" v-model="info.isdispshop" id="group42" value="F"><label for="group42">미노출</label>
                                    </div>
                                </td>
                            </tr>
                            <tr v-show="info.isdispshop === 'T'">
                                <th>쇼핑메인 이미지(PC)<i class="essential"></i></th>
                                <td>
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 620px; height: 457px;" :class="{'no-image': $util.isNull(files['spcimgfile'])}"><!-- 이미지 없는 경우 no-image 클래스 추가 -->
                                            <img style="width: 620px; height: 457px;" :src="imgPreview['spcimgfile']" alt="쇼핑메인 이미지(pc)" v-if="!$util.isNull(files['spcimgfile'])">
                                        </div>
                                        <button type="button" class="btn blue-line mt10" style="width: 100%;"
                                            v-if="$util.isNull(files['spcimgfile'])" @click="fileAttach('spcimgfile')">파일 올리기</button>
                                        <input type="file" ref="spcimgfile" @change="handleFileUpload('spcimgfile')" accept="image/jpeg, image/png" hidden/>
                                        <button type="button" class="btn blue-line mt10" style="width: calc(50% - 3px);"
                                            v-if="!$util.isNull(files['spcimgfile'])" @click="fileAttach('spcimgfile')">변경</button>
                                        <button type="button" class="btn red-line mt10" style="width: calc(50% - 4px);"
                                            v-if="!$util.isNull(files['spcimgfile'])" @click="removeFile('spcimgfile')">삭제</button>
                                    </div>
                                    <div class="img-with-text text">
                                        <p class="txt-orange"><i class="icon-alert"></i>대표 이미지입니다. 보기 쉬운 간결한 이미지를 활용해 주세요.</p>
                                        <p class="txt-orange"><i class="icon-alert"></i>사이즈: 620*457 / 용량: 10MB 이하 / 파일 : JPG, JPEG, PNG</p>
                                    </div>
                                </td>
                            </tr>
                            <tr v-show="info.isdispshop === 'T'">
                                <th>쇼핑메인 이미지(모바일)<i class="essential"></i></th>
                                <td>
                                    <div class="mb10">
                                        <input type="checkbox" v-model="info.copyimgcheck2" @change="setSameAsPcepreImg2" id="copy-img2"><label for="copy-img2">PC 대표 이미지를 복사</label>
                                    </div>
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 720px; height: 640px;" :class="{'no-image': $util.isNull(files['smobileimgfile'])}">
                                            <img style="width: 720px; height: 640px;" :src="imgPreview['smobileimgfile']" alt="이미지(모바일)" v-if="!$util.isNull(files['smobileimgfile'])">
                                        </div>
                                        <button type="button" class="btn blue-line mt10" style="width: 100%;"
                                            v-if="$util.isNull(files['smobileimgfile'])" @click="fileAttach('smobileimgfile')">파일 올리기</button>
                                        <input type="file" ref="smobileimgfile" @change="handleFileUpload('smobileimgfile')" accept="image/jpeg, image/png" hidden/>
                                        <button type="button" class="btn blue-line mt10" style="width: calc(50% - 3px);"
                                            v-if="!$util.isNull(files['smobileimgfile'])" @click="fileAttach('smobileimgfile')">변경</button>
                                        <button type="button" class="btn red-line mt10" style="width: calc(50% - 4px);"
                                            v-if="!$util.isNull(files['smobileimgfile'])" @click="removeFile('smobileimgfile')">삭제</button>
                                    </div>
                                    <div class="img-with-text text">
                                        <p class="txt-orange"><i class="icon-alert"></i>모바일 리스팅 및 와이드형 화면에 노출되는 이미지를 업로드 해 주세요.</p>
                                        <p class="txt-orange"><i class="icon-alert"></i>사이즈: 720*640 / 용량: 10MB 이하 / 파일 : JPG, JPEG, PNG</p>
                                    </div>
                                </td>
                            </tr>
                            <tr v-show="info.isdispshop === 'T'">
                                <th>컨텐츠 키워드<i class="essential"></i></th>
                                <td>
                                    <div class="mb10">
                                        <input type="text" placeholder="키워드 입력" ref="keyword">
                                        <button type="button" class="btn blue-line ml3" @click="addKeyword">추가</button>
                                    </div>
                                    <div class="keyword-group" v-for="(row,key) in this.keywordlist" :key="key">
                                        <div>{{ row.keyword }}<button type="button" class="file-del" @click="removeKeyword(key)"></button></div>
                                    </div>                                    
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="btn-group">
                    <button type="button" class="btn big blue" @click="goSave">저장</button>
                    <button type="button" class="btn big darkgray" @click="onClose">취소</button>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
import CommonDatePickerFromTo from "@views.admin/common/CommonDatePickerFromTo";
import CommonEditor from "@views.admin/common/CommonEditor";
export default {
    props: {
        commonCodeList : Object,
        idx: Number
    },
    name: 'admin.operation.magazine.trend.detail',
    components: {
        CommonDatePickerFromTo,
        CommonEditor
    },
    data() {
        return {
            info: {
                isdisplay: 'F',
                subject: '',
                desc: '',
                trsttime: '',
                tredtime: '',
                mzcateidx: '',
                contents: '',
                mobile_content: '',
                isdispshop: '',
                copyimgcheck: false,
                copyimgcheck2: false,
            },
            keywordlist: [],
            deletekeywordlist: [],
            categorylist: [],
            timeInfo: {              // 진행기간
                startYYYYMMDD: '',
                startHH: '',
                startMM: '',
                toYYYYMMDD: '',
                toHH: '',
                toMM: ''
            },
            checkObj: {
                muappchtypearr: [],
                isallmuappch: 'F',
                mumembertypearr: [],
                isallmumember: 'F',
                mumemlvtypearr: [],
                isallmumemlv: 'F',
            },
            commonCode: this.commonCodeList,
            files: {},
            addfiles: {
                pcimgfile: '',
                mobileimgfile: '',
                spcimgfile: '',
                smobileimgfile: '',
            },
            imgPreview: {
                pcimgfile: '',
                mobileimgfile: '',
                spcimgfile: '',
                smobileimgfile: '',
            },
            deletefile: [],
            isRead: false,
            isWrite: false,
        }
    },
    mounted() {
        this.$http.post('/admin/common/pageAuth/check', {url : this.$options.name}).then(result => {

            this.isRead = (result.data.isread === 'T');
            this.isWrite = (result.data.iswrite === 'T');
      
            if(this.isRead) {
                let param = {
                    tridx : this.idx
                }

                this.$http.post("/admin/operation/magazine/trend/detail", param)
                    .then(result =>{
                        this.info = result.data.info;
                        this.files = result.data.files;
                        this.keywordlist = result.data.keywordlist;
                        if(this.$util.isNull(this.keywordlist)) {
                            this.keywordlist = [];
                        }
                        this.categorylist = result.data.categorylist;
                        this.imgPreview.pcimgfile = this.files.pcimgfile.fullpath;
                        this.imgPreview.mobileimgfile = this.files.mobileimgfile.fullpath;
                        if(this.info.isdispshop === 'T') {
                            this.imgPreview.spcimgfile = this.files.spcimgfile.fullpath;
                            this.imgPreview.smobileimgfile = this.files.smobileimgfile.fullpath;
                        } else {
                            this.$set(this.files,'spcimgfile','');
                            this.$set(this.files,'smobileimgfile','');
                        }
                        this.$refs.editor.content = this.info.contents;
                        this.$refs.moeditor.content = this.info.mobile_content;
                        this.info.copyimgcheck = false;
                        this.info.copyimgcheck2 = false;
                        this.setDateInit();
                    })
                    .catch(error => {
                        this.$util.debug(error);
                    });
            } else{
                alert('페이지 접근 권한이 없습니다.');
                this.onClose();
            }

            if(!this.isWrite){
                let buttons = this.$el.getElementsByTagName('button');

                for(let button of buttons){
                    if(button.className !== 'pop-close') {
                        button.style.display = 'none';
                        button.disabled = true;
                    }
                }
            }
        }).catch(error => {
            this.$util.debug(error);
        });
    },
    methods: {
        setDateInit() {
            this.timeInfo.startYYYYMMDD = this.$util.getFormatDate(this.info.trsttime.substring(0,8),'-');
            this.timeInfo.startHH = this.info.trsttime.substring(8,10);
            this.timeInfo.startMM = this.info.trsttime.substring(10,12);
            this.timeInfo.toYYYYMMDD = this.$util.getFormatDate(this.info.tredtime.substring(0,8),'-');
            this.timeInfo.toHH = this.info.tredtime.substring(8,10);
            this.timeInfo.toMM = this.info.tredtime.substring(10,12);

            if(!this.$util.isNull(this.info.muappchtype)) {
                this.checkObj.muappchtypearr = this.info.muappchtype.split(',');
            }
            if(!this.$util.isNull(this.info.mumembertype)) {
                this.checkObj.mumembertypearr = this.info.mumembertype.split(',');
            }
            if(!this.$util.isNull(this.info.mumemlvtype)) {
                this.checkObj.mumemlvtypearr = this.info.mumemlvtype.split(',');
            }
        },
         // 진행기간 DatePicker 콜백 메서드
        getEventTimeDate(date) {
            this.info.trsttime = date.fromDate12;
            this.info.tredtime = date.toDate12;
        },
        goSave() {
            if(this.checkValidation()) {
                let params = this.info;
                let files = [];
                if(!this.$util.isNull(this.addfiles.pcimgfile)){
                    files.push({key: 'pcimgfile', file: this.addfiles.pcimgfile.file});
                }
                if(!this.$util.isNull(this.addfiles.mobileimgfile)){
                    // pc이미지 복사를 한 경우 file key가 없고 복사시 기존에 저장되어있는 pcimgfile을 저장해야하므로 pcimgfile의 idx를 보냄
                    if(Object.prototype.hasOwnProperty.call(this.files.mobileimgfile,'file')){
                        files.push({key: 'mobileimgfile', file: this.addfiles.mobileimgfile.file});
                    } else {
                        params.copycheck = true;
                        params.copyidx = this.files.pcimgfile.idx;
                    }
                }

                if(this.info.isdispshop === 'T') {
                    if(!this.$util.isNull(this.addfiles.spcimgfile)){
                        files.push({key: 'spcimgfile', file: this.addfiles.spcimgfile.file});
                    }
                    if(!this.$util.isNull(this.addfiles.smobileimgfile)){
                        // pc이미지 복사를 한 경우 file key가 없고 복사시 기존에 저장되어있는 pcimgfile을 저장해야하므로 pcimgfile의 idx를 보냄
                        if(Object.prototype.hasOwnProperty.call(this.files.smobileimgfile,'file')){
                            files.push({key: 'smobileimgfile', file: this.addfiles.smobileimgfile.file});
                        } else {
                            params.scopycheck = true;
                            params.scopyidx = this.files.spcimgfile.idx;
                        }
                    }

                    let addkeywordlist = this.keywordlist.filter(a => !Object.prototype.hasOwnProperty.call(a,'mztrkidx'));
                    if(addkeywordlist.length > 0) {
                        params.addkeywordlist = addkeywordlist;
                    }
                    if(this.deletekeywordlist.length > 0) {
                        params.deletekeywordlist = this.deletekeywordlist;
                    }
                } else {
                    if(!this.$util.isNull(this.files.spcimgfile)) {
                        if(Object.prototype.hasOwnProperty.call(this.files.spcimgfile,'idx')){
                            this.deletefile.push(this.files.spcimgfile.idx);
                        }
                    }
                    if(!this.$util.isNull(this.files.smobileimgfile)) {
                        if(Object.prototype.hasOwnProperty.call(this.files.smobileimgfile,'idx') && !this.info.copyimgcheck2){
                            this.deletefile.push(this.files.smobileimgfile.idx);
                        }
                    }

                    let deletekeyword = this.deletekeywordlist;
                    this.keywordlist.forEach(item => {
                        if(Object.prototype.hasOwnProperty.call(item,'mztrkidx')) {
                            deletekeyword.push(item.mztrkidx);
                        }
                    })

                    if(deletekeyword.length > 0) {
                        params.deletekeywordlist = deletekeyword;
                    }
                }
                params.files = files;
                params.deletefile = new Set(this.deletefile);

                let param = {
                    idx: this.idx,
                    overmsg: '저장하시겠습니까?',
                    zeromsg: '저장',
                    isdisplay: this.info.isdisplay,
                    isdispshop: this.info.isdispshop,
                    trsttime: params.trsttime,
                    tredtime: params.tredtime,
                    isloading: false,
                }

                this.$http.post("/admin/operation/magazine/trend/check", param)
                .then(result => {
                    if (result.statusCode == 200) {
                        params.msg = result.data.msg;
                        this.update(params);
                    }
                })
                .catch(error => {
                    this.$util.debug(error);
                })
            }
        },
        update(params) {
            if(confirm(params.msg)){
                this.$http.post("/admin/operation/magazine/trend/modify", params)
                .then(result => {
                    if(result.statusCode === 200){
                        alert("저장되었습니다.");
                        this.onClose(true);
                    }
                })
                .catch(error => {
                    this.$util.debug(error);
                });
            }   
        },
        checkValidation() {
            this.setEditorText();

            let msg = '';
            let valid = [
                {field: 'info.subject', type: 'I', name: '제목', required: true},
                {field: 'info.contents', type: 'I', name: '내용(PC)', required: true},
                {field: 'info.mobile_content', type: 'I', name: '내용(모바일)', required: true},
                {field: 'info.isdispshop', type: 'S', name: '쇼핑메인', required: true},
                {field: 'info.mzcateidx', type: 'S', name: '카테고리', required: true},
                {field: 'info.muappchtype', type: 'S', name: '노출채널', required: true},
                {field: 'info.mumembertype', type: 'S', name: '대상회원유형', required: true},
                {field: 'info.mumemlvtype', type: 'S', name: '대상회원등급', required: true},
            ];

            msg = this.$util.validMsg(this.$data, valid);
            if(!this.$util.isNull(msg)){
                alert(msg);
                return false;
            }

            if(this.info.trsttime.length < 12) {
                alert("전시시작시간을 선택해주세요.");
                return false;
            }
            if(this.info.tredtime.length < 12) {
                alert("전시종료시간을 선택해주세요.");
                return false;
            }
            if(this.info.trsttime >= this.info.tredtime) {
                alert("전시종료시간을 시작시간 이후로 설정해주세요.");
                return false;
            }

            if(this.files.pcimgfile === ''){
                alert("이미지(PC)를 등록해주세요.");
                return false;
            }
            if(this.files.mobileimgfile === ''){
                alert("이미지(모바일)를 등록해주세요.");
                return false;
            }

            if(this.info.isdispshop === 'T') {
                if(this.files.spcimgfile === ''){
                    alert("쇼핑메인 이미지(PC)를 등록해주세요.");
                    return false;
                }
                if(this.files.smobileimgfile === ''){
                    alert("쇼핑메인 이미지(모바일)를 등록해주세요.");
                    return false;
                }
            }

            if(this.keywordlist.length < 3 && this.info.isdispshop === 'T') {
                alert("컨텐츠 키워드 3개를 등록해주세요.");
                return false;
            }

            return true;
        },
        // 첨부파일(탐색기 열기)
        fileAttach: function(fileTypeKey) {
            if (Array.isArray(this.$refs[fileTypeKey])) {
                this.$refs[fileTypeKey][0].click();
            } else {
               this.$refs[fileTypeKey].click();
            }
        },
        // 가져온 파일 세팅
        handleFileUpload: function(fileTypeKey, target) {
            // PC, 모바일 대표이미지
            let file = this.$refs[fileTypeKey];
            if (this.$util.isNull(file)) {
                return;
            }

            if(Object.prototype.hasOwnProperty.call(this.files[fileTypeKey],'idx')){
                if((fileTypeKey !== 'mobileimgfile' && fileTypeKey !== 'smobileimgfile') || (!this.info.copyimgcheck && !this.info.copyimgcheck2)) {
                    this.deletefile.push(this.files[fileTypeKey].idx);
                }
            }

            let fileType = ['image/png','image/jpeg', 'image/png'];
            if(!fileType.includes(file.files[0].type)){
                alert('jpg, jpeg, png파일만 첨부 가능합니다.');
                file.value = null;
                this.files[fileTypeKey] = '';
                return false;
            }
            if(file.files[0].size > 1048576 * 10){
                alert('파일 최대 크기는 10MB를 초과 할 수 없습니다.');
                file.value = null;
                this.files[fileTypeKey] = '';
                return false;
            }
            let fileObj = {
                file: file.files[0],
                iscreated: true
            }
            this.files[fileTypeKey] = fileObj;
            this.addfiles[fileTypeKey] = fileObj;
            this.imgPreview[fileTypeKey] = URL.createObjectURL(fileObj.file);

            if(fileTypeKey === 'pcimgfile' && this.info.copyimgcheck){
                this.setSameAsPcepreImg();
            }
            if(fileTypeKey === 'spcimgfile' && this.info.copyimgcheck2){
                this.setSameAsPcepreImg2();
            }
            // 모바일 대표이미지 변경시 PC이미지와 동일 체크 해제
            if (fileTypeKey === 'mobileimgfile') {
                this.info.copyimgcheck = false;
            }
            if (fileTypeKey === 'smobileimgfile') {
                this.info.copyimgcheck2 = false;
            }
        },
        removeFile(fileTypeKey) {
            if(confirm('파일을 삭제 하시겠습니까?')){
                if(Object.prototype.hasOwnProperty.call(this.files[fileTypeKey],'idx')){
                    if((fileTypeKey !== 'mobileimgfile' && fileTypeKey !== 'smobileimgfile') || (!this.info.copyimgcheck && !this.info.copyimgcheck2)) {
                        this.deletefile.push(this.files[fileTypeKey].idx);
                    }
                }

                this.files[fileTypeKey] = '';
                this.addfiles[fileTypeKey] = '';
                this.imgPreview[fileTypeKey] = '';
                this.$refs[fileTypeKey].value = null;

                // 모바일 대표이미지 변경시 PC이미지와 동일 체크 해제
                if (fileTypeKey == 'mobileimgfile') {
                    this.info.copyimgcheck = false;
                }
                if (fileTypeKey == 'smobileimgfile') {
                    this.info.copyimgcheck2 = false;
                }
            }
        },
        // PC 대표이미지와 동일하게 세팅
        setSameAsPcepreImg: function() {
            if(Object.prototype.hasOwnProperty.call(this.files.mobileimgfile,'idx')){
                if(this.info.copyimgcheck) {
                    if(!this.deletefile.includes(this.files.mobileimgfile.idx)) {
                        this.deletefile.push(this.files.mobileimgfile.idx);
                    }
                }
            }
            if (this.info.copyimgcheck) {
                this.files.mobileimgfile = this.files.pcimgfile;
                this.addfiles.mobileimgfile = this.files.pcimgfile;
                this.imgPreview.mobileimgfile = this.imgPreview.pcimgfile;
            } else {
                this.files.mobileimgfile = '';
                this.addfiles.mobileimgfile = '';
                this.imgPreview.mobileimgfile = '';
            }
        },
        setSameAsPcepreImg2: function() {
            if(Object.prototype.hasOwnProperty.call(this.files.smobileimgfile,'idx')){
                if(this.info.copyimgcheck2) {
                    if(!this.deletefile.includes(this.files.smobileimgfile.idx)) {
                        this.deletefile.push(this.files.smobileimgfile.idx);
                    }
                }
            }
            if (this.info.copyimgcheck2) {
                this.files.smobileimgfile = this.files.spcimgfile;
                this.addfiles.smobileimgfile = this.files.spcimgfile;
                this.imgPreview.smobileimgfile = this.imgPreview.spcimgfile;
            } else {
                this.files.smobileimgfile = '';
                this.addfiles.smobileimgfile = '';
                this.imgPreview.smobileimgfile = '';
            }
        },
        // 조회조건 - 적용채널 전체체크
        checkAllAppchtype: function() {
            let isAllCheck = this.checkObj.isallmuappch;
            this.checkObj.muappchtypearr = [];
            if (isAllCheck == 'T') {
                for(let type of this.commonCode.muappchtype){
                    this.checkObj.muappchtypearr.push(type.cmcode);
                }
            }
        },
        // 조회조건 - 적용채널 전체체크
        checkAllMembertype: function() {
            let isAllCheck = this.checkObj.isallmumember;
            this.checkObj.mumembertypearr = [];
            if (isAllCheck == 'T') {
                for(let type of this.commonCode.mumembertype){
                    this.checkObj.mumembertypearr.push(type.cmcode);
                }
            }
        },
        // 조회조건 - 적용채널 전체체크
        checkAllMemLvtype: function() {
            let isAllCheck = this.checkObj.isallmumemlv;
            this.checkObj.mumemlvtypearr = [];
            if (isAllCheck == 'T') {
                for(let type of this.commonCode.mumemlvtype){
                    this.checkObj.mumemlvtypearr.push(type.cmcode);
                }
            }
        },
        // 에디터 내용 가져오기
        setEditorText() {
            if (this.$util.isNull(this.$refs.editor.content)) {
                this.info.contents = ''
            } else {
                this.info.contents = this.$refs.editor.content;
            }

            if (this.$util.isNull(this.$refs.moeditor.content)) {
                this.info.mobile_content = ''
            } else {
                this.info.mobile_content = this.$refs.moeditor.content;
            }
        },
        // PC 내용 복사
        copyByContent() {
            this.$refs.moeditor.content = this.$refs.editor.content;
        },
        addKeyword() {
            let obj = {
                keyword: this.$refs.keyword.value,
            }; 

            if(this.$util.isNull(obj.keyword)) {
                alert("추가할 키워드를 입력해주세요.");
                return;
            }

            if(this.keywordlist.length === 3) {
                alert("키워드는 3개까지만 등록가능합니다.");
                return;
            }

            this.keywordlist.push(obj);
            this.$refs.keyword.value = '';
        },
        removeKeyword(index) {
            if(Object.prototype.hasOwnProperty.call(this.keywordlist[index],'mztrkidx')) {
                this.deletekeywordlist.push(this.keywordlist[index].mztrkidx);
            }
            this.keywordlist.splice(index, 1);
        },
        onClose(isreload) {
            this.info = this.$options.data().info;
            this.keywordlist = this.$options.data().keywordlist;
            this.deletekeywordlist = this.$options.data().deletekeywordlist;
            this.timeInfo = this.$options.data().timeInfo;
            this.files = this.$options.data().files;
            this.imgPreview = this.$options.data().imgPreview;
            this.$refs.editor.content = "";
            this.$refs.moeditor.content = "";
            // 팝업 닫기
            if (typeof(isreload)==='boolean' && isreload) {
                this.$emit('closeDetail', isreload);
            } else {
                this.$emit('closeDetail');
            }
        },
    },
    watch: {
        'checkObj.muappchtypearr': function(value){
            if (value.length < this.commonCode.muappchtype.length) {
                this.checkObj.isallmuappch = 'F';
            } else {
                this.checkObj.isallmuappch = 'T';
            }
            this.info.muappchtype = this.checkObj.muappchtypearr.join();
        },
        'checkObj.mumembertypearr': function(value){
            if (value.length < this.commonCode.mumembertype.length) {
                this.checkObj.isallmumember = 'F';
            } else {
                this.checkObj.isallmumember = 'T';
            }
            this.info.mumembertype = this.checkObj.mumembertypearr.join();
        },
        'checkObj.mumemlvtypearr': function(value){
            if (value.length < this.commonCode.mumemlvtype.length) {
                this.checkObj.isallmumemlv = 'F';
            } else {
                this.checkObj.isallmumemlv = 'T';
            }
            this.info.mumemlvtype = this.checkObj.mumemlvtypearr.join();
        },
    }
}
</script>

<style>

</style>