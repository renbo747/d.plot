...
export default {
    name: 'admin.promotion.coupon.coupondetail',
    ...
    data() {
        return {
            ...
            sortData: {
                dadamembertypename: 'dadamembertypename_desc',
                memlvtypename: 'memlvtypename_desc',
                issueday: 'issueday_desc',
                useday: 'useday_desc'
            },
            info: {
                comcpnidx: '',      // 쿠폰일련번호
                ...
                couponissueList: [],        // 쿠폰발급이력
                couponmemissueList: [],      // 쿠폰발급사용이력
                couponmemissueListCnt: 0     // 쿠폰발급사용이력 총 건수
            },
            ...
            issearchMemissue: false,    // 발급사용이력 조회여부
            isDisabled : false,         // disable 여부
            isTrashDisabled : false,    // 사용여부 disable 여부
            ...
            .then(result => {
                ...
                if(!this.isWrite){
                    ...
                    for(let button of buttons){
                        if(button.className !== 'pop-close' && button.id === 'copy') {
    ...
    methods: {
        ...
        disableButtons: function() {
            let buttons = this.$el.getElementsByTagName('button');
            let showButtonsId = ['close', 'cancel', 'urlcopy', 'excel','save','itemAdd1','itemAdd1','itemRemove1','itemRemove2'];
            for(let button of buttons){
        ...
        getCouponDetail: function() {
            let params = {
                comcpnidx: this.activeComcpnidx, 
                cpninfoidx: this.activeCpninfoidx
            };
            params = Object.assign(params, this.memberPagingData);
            this.$http.post('/admin/promotion/coupon/detail', params)
                .then(result => {
                    ...
                    this.info.orgisautopay = this.info.isautopay

                    if(!this.$util.isNull(this.info.issendmsg) && this.info.issendmsg > 0) {
                        this.info.issendmsgyn = 'Y';
                    }
                    // 정기발급발급시분이 있는 경우 세팅
                    ...
                    // 정기발급 발급전 쿠폰이 하나이상이고 전체쿠폰수보다 발급전쿠폰수가 적을때 사용여부만 수정 가능
                    if(this.info.cpnissuetype === this.$store.getters['ADMIN'].CPN_ISSUE_REGULAR && this.info.bfissuecnt > 0 && this.info.totcpncnt > this.info.bfissuecnt) {
                        this.isDisabled = true;
                    } else if (this.info.cpnissuetype === this.$store.getters['ADMIN'].CPN_ISSUE_REGULAR && this.info.bfissuecnt === 0) {
                        this.isDisabled = true;
                        this.isTrashDisabled = true;
                    } else if (this.info.cpnissuest != this.$store.getters['ADMIN'].CPN_ISSUE_ST_BEFORE) {
                        this.isDisabled = true;
                        this.isTrashDisabled = true;
                    }
                    this.searchMemberIssueList(true);

                    setTimeout(function () {
                        // 진행상태에 따른 버튼 노출여부
                        if(this.isWrite && this.info.cpnissuest != this.$store.getters['ADMIN'].CPN_ISSUE_ST_BEFORE) {
                            this.disableButtons();
                            // if(!(this.info.cpnissuetype === this.$store.getters['ADMIN'].CPN_ISSUE_REGULAR && this.info.bfissuecnt > 0 && this.info.totcpncnt > this.info.bfissuecnt)) {
                            //     this.disableButtons();
                            // }
        ...
        // 쿠폰 발급/사용이력 목록 조회
        searchMemberIssueList: function(isInit) {
            this.issearchMemissue = true;
            let params = {
                comcpnidx: this.activeComcpnidx, 
                cpninfoidx: this.activeCpninfoidx,
                comcpntype : this.info.comcpntype,
                isautopay : this.info.isautopay
            };
            params = Object.assign(params, this.memberPagingData);
            this.$http.post('/admin/promotion/coupon/detail/memissue/list', params)
                .then(result => {
                    this.$util.debug(result);
                    let data = result.data;
                    if (isInit) {
                        this.info.couponmemissueList = data.list;
                        this.info.couponmemissueListCnt = data.count;
                        this.$refs.memberScrollTarget.scrollTop = 0;
                    } else {
                        this.info.couponmemissueList = this.info.couponmemissueList.concat(data.list);
                    }
                    
                    setTimeout(function () {
                        this.issearchMemissue = false;
                    }.bind(this), 50);
                })
                .catch(error => {
                    this.$util.debug(error);
        ...
        // 스크롤페이징
        onMemberScroll: function({ target: { scrollTop, clientHeight, scrollHeight }}) {
            // 테이블 스크롤이 바닥인 경우
            if (!this.issearchMemissue && Number(scrollTop) + Number(clientHeight) + 37.08 >= Number(scrollHeight)) {
                if (this.info.couponmemissueListCnt > this.info.couponmemissueList.length) {
                    this.memberPagingData.page++;
                    this.searchMemberIssueList(false);
                }
            }
        }, 
        // 다운로드 스크립트 팝업 오픈
        ...
        sortToggle(key){
            ...
            this.info.couponmemissueList.sort((a, b) => {
        ...
        downloadExcel: function() {
            if (this.info.couponmemissueList.length == 0) {
                alert('다운로드할 내역이 존재하지 않습니다.');
                return;
            }
            let config = { responseType: 'arraybuffer' };
            this.$http.post('/admin/promotion/coupon/memissue/exceldown', { cpninfoidx: this.info.cpninfoidx, comcpntype : this.info.comcpntype, isautopay : this.info.isautopay}, config)
                .then(result => {
                    this.$util.debug(result);
                })
                .catch(error => {
        ...
        checkValidation: function() {
            ...
            if (this.info.cpnissuetype == this.$store.getters['ADMIN'].CPN_ISSUE_REGULAR) {
                ...
                if(checkResult && (this.info.evrcpnuseedday <= this.info.evrcpnusestday)) {
                    checkResult = false;
                    alert('[사용조건] 정기발급기간 종료일시를 사용시작일시 이후로 입력하세요.');
                }
            }
            // 사용시작일시(사용기간종류-기간설정)
            else {
            ...
            if(this.info.isautopay=='F') {
                ...
                if (checkResult && this.info.isdupperson=='T' && this.$util.isNull(this.info.dupcnt)) {
                    checkResult = false;
                    alert('[발급조건] 동일인 재발급수량을 선택해주세요.');
                }
            }
        ...
        save: function() {
            if (this.checkValidation()) {
                if(confirm('저장하시겠습니까?')) {
                    this.$http.post('/admin/promotion/coupon/update', this.info)
                        .then(result => {
                            this.$util.debug(result);
                            if (result.statusCode =='200') {
                                alert('저장이 완료되였습니다.');
