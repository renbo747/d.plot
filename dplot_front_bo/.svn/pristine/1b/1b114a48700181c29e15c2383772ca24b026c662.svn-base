<template>
    <div class="content m-leftmenu">
        ...
        <ShopExFiveRegist v-if="showRegist" v-bind:commonCodeList="commonCode" v-on:closeRegist="closeRegist"></ShopExFiveRegist>
        <ShopExFiveDetail v-if="showDetail" :commonCodeList="commonCode" :idx="idx" v-on:closeDetail="closeDetail"></ShopExFiveDetail>
        <div class="inner">
            <div class="boxing search-area">
                ...
                <dl>
                    ...
                    <dd>
                        <select v-model="searchData.stype">
                            <option value="">전체</option>
                            <option value="exsttime">시작일자</option>
                            <option value="exedtime">종료일자</option>
                ...
                <dl>
                    ...
                    <dd>
                        <div class="radio_wrap ">
                            <input type="radio" name="group06" v-model="searchData.ex5type" id="group61" value="" checked><label for="group61">전체</label>
                            <input type="radio" name="group06" v-model="searchData.ex5type" id="group62" value="EX5001"><label for="group62">A (PC : 좌, 모바일 : 상)</label>
                            <input type="radio" name="group06" v-model="searchData.ex5type" id="group63" value="EX5002"><label for="group63">B (PC : 우, 모바일 : 하)</label>
                        </div>
                    </dd>
                </dl>
                <dl>
            ...
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                    ...
                    <tr>
                        ...
                        <th>시작일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.exsttime"
                                    class="sort"
                                    :class="[{up : sortData.exsttime === 'exsttime_asc'}, {down : sortData.exsttime === 'exsttime_desc'}]"
                                    @click="sortToggle(sortData.exsttime)">
                            </button>
                        </th>
                        <th>종료일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.exedtime"
                                    class="sort"
                                    :class="[{up : sortData.exedtime === 'exedtime_asc'}, {down : sortData.exedtime === 'exedtime_desc'}]"
                                    @click="sortToggle(sortData.exedtime)">
                            </button>
                        </th>
                        <th>등록일자
                            <button type="button" v-if="isRead" 
                ...
                <tbody v-if="this.list.length > 0">
                    <tr v-for="(row, index) in this.list" :key="index">
                        <td><input type="checkbox" v-model="checkedList" :id="row.ex45idx" :value="row.ex45idx" @change="checkList($event.target.checked)"/></td>
                        <td>{{ $util.isNull(row.ex5type) ? '-' : row.ex5type}}</td>
                        <td class="left"><a class="link" @click="goDetail(row.ex45idx)">{{ row.subject }}</a></td>
                        <td>
                            <div class="img-thumb size120" :class="$util.isNull(row.pcimgurl) ? 'no-image' : ''" style="margin: 0 auto;">
                                <img :src="row.pcimgurl">
                            </div>
                        </td>
                        <td>
                            <div class="img-thumb size120" :class="$util.isNull(row.mobileimgurl) ? 'no-image' : ''"  style="margin: 0 auto;">
...
import ShopExFiveRegist from './ShopExFiveRegist.vue';
import ShopExFiveDetail from './ShopExFiveDetail.vue';
export default {
    ...
    data() {
        return {
            ...
            sortData: {
                exsttime: 'exsttime_desc',              // 시작일자 정렬기준
                exedtime: 'exedtime_desc',                // 시작일자 정렬기준
    ...
    methods: {
        ...
        searchList(page) {
            ...
            this.$http.post("/admin/operation/shopping/shopexfive/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
        ...
        showChange(show) {
            ...
            this.$http.post("/admin/operation/shopping/shopexfive/check", params)
            .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/shopping/shopexfive/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        checkAllList: function(value) {
            ...
            if (value) {
                this.list.forEach(item => {
                    this.checkedList.push(item.ex45idx);
                });
            }
        },
        // 목록 개별체크
        ...
        fnExcelDownload: function() {
            ...
            this.$http.post('/admin/operation/shopping/shopexfive/excel', this.searchData, config)
            .then(result => {
