<template>
    ...
    <div class="content m-leftmenu">
        ...
        <div class="inner">
            <div class="clearfix">
                <div class="bar-title fl">배송완료(구매확정)</div>
            </div>
                ...
                <dl>
                    ...
                    <dd>
                        <select v-model="searchData.dtkey">
                            ...
                            <option value="deliv">배송완료일</option>
                        </select>
                ...
                <dl>
                    ...
                    <dd>
                        <div class="check-wrap">
                            <input type="checkbox" id="chkAllStatus" v-model="searchData.isallstatus" true-value="T" false-value="F" @change="checkAllOrdstatus">
                            <label for="chkAllStatus">전체</label>
                        </div>
                        <div class="check-wrap" v-for="item in commonCode.ordstatus" :key="item.cmcode">
                            <input type="checkbox" :id="'ordstatus_'+item.cmcode" v-model="searchData.ordstatusArr" :true-value="[]" :value="item.cmcode"/>
                            <label :for="'ordstatus_'+item.cmcode">{{ item.codename }}</label>
                        </div>
                    </dd>
                </dl>
                <dl>
                    ...
                    <dd>
                        <div class="check-wrap">
                            <input type="checkbox" id="chkAllOrdcfm" v-model="searchData.isallordcfm" true-value="T" false-value="F" @change="checkAllordcfmtype">
                            <label for="chkAllOrdcfm">전체</label>
                        </div>
                        <div class="check-wrap" v-for="item in commonCode.ordcfmtype" :key="item.cmcode">
                            <input type="checkbox" :id="'ordcfmtype_'+item.cmcode" v-model="searchData.ordcfmtypeArr" :true-value="[]" :value="item.cmcode"/>
                            <label :for="'ordcfmtype_'+item.cmcode">{{ item.codename }}</label>
                        </div>
                    </dd>
            ...
            <div class="caption-group mt10 clearfix">
                ...
                <div class="btn-group fr">
                    <button type="button" v-if="isWrite && !isPartner" class="btn blue-line" @click="fnChangeOrdStatus($store.getters['ADMIN'].ORDER_PURCH_CONFIRM)">구매확정처리</button>
            ...
            <div class="scroll-x">
                <table cellpadding="0" cellspacing="0" class="data-tb align-c" :style="!isPartner? 'width: 3700px;':'width: 2800px;'">
                    ...
                    <thead>
                        <tr>
                            <th v-if="!isPartner"><input type="checkbox" id="chkall" v-model="isallchk" @change="checkAllList($event.target.checked)"/></th>
                    ...
                    <tbody v-if="list.length > 0">
                        <tr v-for="item in list" :key="item.orgdelividx">
                            <td v-if="!isPartner"><input type="checkbox" :id="item.orgdelividx" v-model="checkedList" :value="item.orgdelividx" :disabled="item.ispbgoods==='T'" @change="checkList($event.target.checked)"/></td>
                    ...
                    <tbody v-else>
                        <tr><td :colspan="isPartner? 24:37">조회 결과가 존재하지 않습니다.</td></tr>
                    </tbody>
                </table>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            searchData: {
                ...
                enddate: '',            // 조회종료일자
                isallstatus: '',        // 전체상태여부
                ordstatusArr: [],       // 주문상태Array
                isallordcfm: '',        // 전체구매확정여부
                ordcfmtypeArr: [],      // 구매확정타입Array
                isallchannel: '',       // 전체채널여부
    ...
    methods: {
        ...
        initSearchData: function () {
            ...
            this.searchData.isallstatus = 'T';
            this.checkAllOrdstatus();
        ...
        getCommonCodeList: function() {
            let cmclassArr = ['MUAPPCHTYPE', 'ORDSTATUS', 'ORDCFMTYPE', 'DADAMEMBERTYPE', 'MEMLVTYPE', 'PAYWAYTYPE'];
            this.$http.post('/common/code/map/list', {cmclass : cmclassArr, isloading: false})
                .then(result =>{
                    ...
                    for (const [key] of Object.entries(data)) {
                        ...
                        if (key === 'ordstatus') {
                            this.commonCode[key].splice(0, 6);
                        }
                        if (key === 'ordcfmtype') {
                            let addItem = {
                                cmclass: 'ORDCFMTYPE',
                            ...
                            this.commonCode[key].splice(0, 0, addItem);
                        }
                    }
                    // 검색조건 초기화
                    this.initSearchData();
                    ...
                    if(this.isLink){
                      ...
                      this.searchData.ordstatusArr = [linkParam.ordstatus];
                    }
                    // 목록 조회
                    this.searchList();
                })
        ...
        // 조회조건 - 상태 전체 체크
        checkAllOrdstatus: function() {
            let isAllCheck = this.searchData.isallstatus;
            this.searchData.ordstatusArr = [];
            if (isAllCheck == 'T') {
                for(let type of this.commonCode.ordstatus){
                    this.searchData.ordstatusArr.push(type.cmcode);
                }
            }
        },
        // 조회조건 - 구매확정 전체 체크
        checkAllordcfmtype: function() {
            let isAllCheck = this.searchData.isallordcfm;
            this.searchData.ordcfmtypeArr = [];
            if (isAllCheck == 'T') {
                for(let type of this.commonCode.ordcfmtype){
                    this.searchData.ordcfmtypeArr.push(type.cmcode);
                }
            }
        },
        // 조회조건 - 대상회원유형 전체체크
        ...
        fnExcelDownload: function() {
            ...
            params.isPartner = this.isPartner;
            params.fileName = '배송완료(구매확정)_주문목록.xlsx';
        ...
        // 주문상태 변경
        fnChangeOrdStatus: function(reqStatus) {
            let params = {
                orderdelivList: [],
                ordstatus: reqStatus
            }
            ...
            for (let i=0; i<this.checkedList.length; i++) {
                ...
                for (let j=0; j<this.list.length; j++) {
                    ...
                    if (orgdelividx === item.orgdelividx) {
                        if (item.ordstatus === this.$store.getters['ADMIN'].ORDER_PURCH_CONFIRM) {
                            alert('이미 구매확정 처리된 내역이 존재합니다. 확인후 진행해주세요.');
                            return;
                        }
                        if (item.ispbgoods === 'T') {
                            alert('파트너사 상품만 구매확정 처리가 가능합니다. 확인후 진행해주세요.');
                            return;
                        }
                        if (item.ordstatus != this.$store.getters['ADMIN'].ORDER_COMPLETE_DELIV) {
                            alert('배송완료된 상품만 구매확정 처리가 가능합니다. 확인후 진행해주세요.');
                            return;
                        }
                        if(item.rtnproccnt > 0) {
                            alert('반품처리가 진행중인 상품은 처리가 불가합니다. 확인후 진행해주세요.');
                            return;
                        }
                        if(item.excproccnt > 0) {
    ...
    watch: {
        ...
        // 상태
        'searchData.ordstatusArr': function(value) {
            if (value.length < this.commonCode.ordstatus.length) {
                this.searchData.isallstatus = 'F';
            } else {
                this.searchData.isallstatus = 'T';
            }
        },
        // 구매확정구분
        'searchData.ordcfmtypeArr': function(value) {
            if (value.length < this.commonCode.ordcfmtype.length) {
                this.searchData.isallordcfm = 'F';
            } else {
                this.searchData.isallordcfm = 'T';
            }
        },
