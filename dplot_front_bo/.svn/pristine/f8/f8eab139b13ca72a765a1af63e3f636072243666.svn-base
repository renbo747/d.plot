                ...
                <div class="boxing">
                    <div class="form-area">
                        ...
                        <dl>
                            ...
                            <dd><input type="text" v-model="info.subject" style="width: 100%" placeholder="팝업 제목" /></dd>
                        </dl>
                        <dl>
                            <dt>설명</dt>
                            <dd><input type="text" v-model="info.expdesc" style="width: 100%" placeholder="팝업 설명" /></dd>
                        </dl>
                    </div>
                </div>
                    ...
                    <table cellpadding="0" cellspacing="0" class="gray-tb">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="radio_wrap">
                                        <input type="radio" name="group05" v-model="info.isdispmagazine" id="group51" value="T" checked=""><label for="group51">매거진 메인</label>
                                        <input type="radio" name="group05" v-model="info.isdispmagazine" id="group52" value="F"><label for="group52">쇼핑 메인</label>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <input type="text" v-model="info.linkpc" style="width: calc(100% - 100px);" placeholder="팝업 클릭 시 연결되는 PC 화면 주소" />
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" v-model="info.linkmo" style="width: calc(100% - 100px);" placeholder="팝업 클릭 시 연결되는 모바일 화면 주소" />
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 250px; height: 310px;" :class="{'no-image': $util.isNull(files['pcimgfile'])}"><!-- 이미지 없는 경우 no-image 클래스 추가 -->
                                            <img style="width: 250px; height: 310px;" :src="imgPreview['pcimgfile']" alt="팝업 이미지(pc)" v-if="!$util.isNull(files['pcimgfile'])">
                                        </div>
                                    ...
                                    <div class="img-with-text text">
                                        ...
                                        <p class="txt-orange"><i class="icon-alert"></i>사이즈: 500*620 / 최소: 200*200 / 용량: 10MB 이하 / 파일 : JPG, JPEG, PNG</p>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb" style="width: 360px; height: 400px;" :class="{'no-image': $util.isNull(files['mobileimgfile'])}">
                                            <img style="width: 360px; height: 400px;" :src="imgPreview['mobileimgfile']" alt="팝업 이미지(모바일)" v-if="!$util.isNull(files['mobileimgfile'])">
                                        </div>
                                        <button type="button" class="btn blue-line mt10" style="width: 100%;"
                                    ...
                                    <div class="img-with-text text">
                                        ...
                                        <p class="txt-orange"><i class="icon-alert"></i>사이즈: 720*800 / 용량: 10MB 이하 / 파일 : JPG, JPEG, PNG</p>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <div class="radio_wrap">
                                        <input type="radio" name="group06" v-model="info.isoption" id="group61" value="F" checked=""><label for="group61">불필요</label>
                                        <input type="radio" name="group06" v-model="info.isoption" id="group62" value="T"><label for="group62">쇼필요 (오늘하루 보지 않기)</label>
                                    </div>
                                </td>
                            </tr> 
                        </tbody>
...
export default {
    name: 'admin.operation.setting.mainpopup.detail',
    props: {
    ...
    mounted() {
        this.$http.post('/admin/common/pageAuth/check', {url : this.$options.name}).then(result => {
            ...
            if(this.isRead){
                ...
                this.$http.post("/admin/operation/setting/mainpopup/detail", param)
                    .then(result =>{
    ...
    methods: {
        setDateInit() {
            this.timeInfo.startYYYYMMDD = this.$util.getFormatDate(this.info.popsttime.substring(0,8),'-');
            this.timeInfo.startHH = this.info.popsttime.substring(8,10);
            this.timeInfo.startMM = this.info.popsttime.substring(10,12);
            this.timeInfo.toYYYYMMDD = this.$util.getFormatDate(this.info.popedtime.substring(0,8),'-');
            this.timeInfo.toHH = this.info.popedtime.substring(8,10);
            this.timeInfo.toMM = this.info.popedtime.substring(10,12);
        ...
        goSave() {
            if(this.checkValidation()) {
                ...
                if(params.isdisplay === 'F') { 
                    ...
                    this.$http.post("/admin/operation/setting/mainpopup/check", param)
                    .then(result => {
        ...
        update(params){
            if(confirm(params.msg)){
                this.$http.post("/admin/operation/setting/mainpopup/modify", params)
                .then(result => {
                    if(result.statusCode === 200){
                        alert("저장되었습니다.");
                        this.onClose(true);
        ...
        checkValidation() {
            ...
            let valid = [
                ...
                {field: 'info.linkpc', type: 'I', name: '링크(PC)', required: true},
                {field: 'info.linkmo', type: 'I', name: '링크(모바일)', required: true},
                {field: 'info.isdispmagazine', type: 'S', name: '노출위치', required: true},
            ...
            if(this.info.popsttime.length < 12) {
                alert("전시시작시간을 선택해주세요.");
                return false;
            }
            if(this.info.popedtime.length < 12) {
                alert("전시종료시간을 선택해주세요.");
                return false;
            }
            if(this.info.popsttime >= this.info.popedtime) {
                alert("전시종료시간을 시작시간 이후로 설정해주세요.");
                return false;
            }

            if(this.files.pcimgfile === ''){
                alert("팝업 이미지(PC)를 등록해주세요.");
                return;
            }
            if(this.files.mobileimgfile === ''){
                alert("팝업 이미지(모바일)를 등록해주세요.");
                return;
            }
        ...
        getEventTimeDate(date) {
            this.info.popsttime = date.fromDate12;
            this.info.popedtime = date.toDate12;
        },
        // 첨부파일(탐색기 열기)
