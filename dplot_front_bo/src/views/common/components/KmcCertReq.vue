<template>
    <div>
        <form ref="reqKMCISForm" name="reqKMCISForm" method="post" action="https://www.kmcert.com/kmcis/web/kmcisReq.jsp">
    ...
    data() {
        return {
            kmcInfo: {
                ...
                plusInfo: "", //추가DATA정보
                tr_cert: "", //요청정보(암호화)
                tr_url: process.env.VUE_APP_KMC_CERT_REDIRECT_URI, //본인인증서비스 결과수신 POPUP URL
  ...
  methods: {
    request(urlCode, succesName ,failName, param) {
        ...
        if (this.$util.isEmpty(param)) {
            ...
            this.$util.debug("plusInfo::::"+this.kmcInfo.plusInfo);
        }
