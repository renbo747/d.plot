<template>
    <div class="content m-leftmenu">
        ...
        <ExhitBannerRegist v-if="showRegist" v-bind:commonCodeList="commonCode" v-on:closeRegist="closeRegist"></ExhitBannerRegist>
        <ExhitBannerDetail v-if="showDetail" :commonCodeList="commonCode" :idx="idx" v-on:closeDetail="closeDetail"></ExhitBannerDetail>
        <div class="inner">
            <div class="boxing search-area">
                ...
                <dl>
                    ...
                    <dd>
                        <div class="radio_wrap">
                            <input type="radio" name="group06" v-model="searchData.exbntype" id="group61" value="" checked><label for="group61">전체</label>
                            <input type="radio" name="group06" v-model="searchData.exbntype" id="group62" value="EBN001"><label for="group62">A (PC : 전시영역2 하단좌, 모바일 : 전시영역2 하단)</label>
                            <input type="radio" name="group06" v-model="searchData.exbntype" id="group63" value="EBN002"><label for="group63">B (PC : 전시영역2 하단우, 모바일 : 전시영역3 하단)</label>
                        </div>
                    </dd>
                </dl>
                <dl>
            ...
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                ...
                <tbody v-if="this.list.length > 0">
                    <tr v-for="(row, index) in this.list" :key="index">
                        <td><input type="checkbox" v-model="checkedList" :id="row.banidx" :value="row.banidx" @change="checkList($event.target.checked)"/></td>
                        <td>{{ loopNumberForPaging(index) }}</td>
                        <td>{{ row.exbntype === 'EBN001' ? 'A' : 'B' }}</td>
...
import ExhitBannerRegist from "./ExhitBannerRegist";
import ExhitBannerDetail from "./ExhitBannerDetail";
export default {
    ...
    methods: {
        ...
        searchList() {
            ...
            this.$http.post("/admin/operation/shopping/exhitbanner/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
        ...
        showChange(show) {
            ...
            this.$http.post("/admin/operation/shopping/exhitbanner/check", params)
            .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/shopping/exhitbanner/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        checkAllList: function(value) {
            ...
            if (value) {
                this.list.forEach(m => {
                    this.checkedList.push(m.banidx);
                });
            }
        },
        // 목록 개별체크
