<template>
    <div class="content m-leftmenu">
        ...
        <TrendRegist v-if="showRegist" v-bind:commonCodeList="commonCode" v-on:closeRegist="closeRegist"></TrendRegist>
        <TrendDetail v-if="showDetail" :commonCodeList="commonCode" :idx="idx" v-on:closeDetail="closeDetail"></TrendDetail>
        <div class="inner">
            <div class="boxing search-area">
                <dl>
                    ...
                    <dd>
                        ...
                        <input type="text" v-model="searchData.sword" @keyup.enter="searchList"/>
                    </dd>
                </dl>
                <dl>
                    <dt>조회기간</dt>
                    <dd>
                        <select v-model="searchData.stype">
                            <option value="">전체</option>
                            <option value="trsttime">시작일자</option>
                            <option value="tredtime">종료일자</option>
            ...
            <div class="btn-group">
                <button type="button" class="btn big blue" v-if="isRead" @click="searchList">검색</button>
            ...
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                ...
                <thead>
                        ...
                        <th>시작일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.trsttime"
                                    class="sort"
                                    :class="[{up : sortData.trsttime === 'trsttime_asc'}, {down : sortData.trsttime === 'trsttime_desc'}]"
                                    @click="sortToggle(sortData.trsttime)">
                            </button></th>
                        <th>종료일자
                            <button type="button" v-if="isRead" 
                                    :value="sortData.tredtime"
                                    class="sort"
                                    :class="[{up : sortData.tredtime === 'tredtime_asc'}, {down : sortData.tredtime === 'tredtime_desc'}]"
                                    @click="sortToggle(sortData.tredtime)">
                            </button>
                        </th>
                        <th>등록일자
                            <button type="button" v-if="isRead" 
                ...
                <tbody v-if="this.list.length > 0">
                    <tr v-for="(row, index) in this.list" :key="index">
                        <td><input type="checkbox" v-model="moveData.targetIdx" :id="row.mzexidx" :value="index" @change="checkList($event.target.checked)"/></td>
                        <td>{{ $util.isNull(row.sortnum) ? '-' : row.sortno }}</td>
                        <td>{{ row.isdispshop === 'F' ? '-' : '●' }}</td>
                        <td class="left"><a class="link" @click="goDetail(row.tridx)">{{ row.subject }}</a></td>
                        <td>
...
import TrendRegist from "./TrendRegist";
...
export default {
    ...
    data() {
        return {
            ...
            sortData: {
                trsttime: 'trsttime_desc',              // 시작일자 정렬기준
                tredtime: 'tredtime_desc',                // 시작일자 정렬기준
    ...
    methods: {
        ...
        searchList() {
            ...
            this.$http.post("/admin/operation/magazine/trend/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
        ...
        showChange(show) {
            ...
            this.moveData.targetIdx.forEach(function(item){
                list.push(thisObj.list[item].tridx);
            });
            ...
            this.$http.post("/admin/operation/magazine/trend/check", params)
            .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/magazine/trend/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        fnExcelDownload: function() {
            ...
            this.$http.post('/admin/operation/magazine/trend/excel', this.searchData, config)
            .then(result => {
