<template>
    <div class="content m-leftmenu">
        ...
        <DisplayRegist v-if="showRegist" v-bind:commonCodeList="commonCode" v-on:closeRegist="closeRegist"></DisplayRegist>
        <DisplayDetail v-if="showDetail" :commonCodeList="commonCode" :idx="idx" v-on:closeDetail="closeDetail"></DisplayDetail>
        <div class="inner">
            ...
            <table cellpadding="0" cellspacing="0" class="data-tb align-c">
                ...
                <tbody v-if="this.list.length > 0">
                    <tr v-for="(row, index) in this.list" :key="index">
                        ...
                        <td class="left"><a class="link" @click="goDetail(row.mzexidx)">{{ row.copy }}</a></td>
                        <td>
...
import DisplayRegist from "./DisplayRegist";
...
export default {
    ...
    methods: {
        ...
        getCommonCodeList: function() {
            let cmclassArr = ['MUAPPCHTYPE', 'MUMEMBERTYPE', 'MUMEMLVTYPE', 'MZTYPE'];
        ...
        searchList(page) {
            ...
            this.$http.post("/admin/operation/magazine/display/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
        ...
        showChange(show) {
            ...
            this.moveData.targetIdx.forEach(function(item){
                list.push(thisObj.list[item].mzexidx);
            });
            ...
            if(show === 'F') { 
                ...
                this.$http.post("/admin/operation/magazine/display/check", params)
                .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/operation/magazine/display/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        fnExcelDownload: function() {
            ...
            this.$http.post('/admin/operation/magazine/display/excel', this.searchData, config)
            .then(result => {
