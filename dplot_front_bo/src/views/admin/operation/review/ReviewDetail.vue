<template>
    <div id="modal-wrap" class="modal" style="display: block;">
        ...
        <div class="modal-content" :style="detailShow || isGoodsDetailShow?'width: 1600px;' :'width: 1300px;'">
            ...
            <div class="pop-body">
                <div class="gray-box mg0 clearfix" v-if="isAdmin">
                    <div class="fl">
                ...
                <div class="bar-title mt10" v-if="isAdmin">작성자 정보</div>
                <div class="form-area" v-if="isAdmin">
                ...
                <div class="form-area">
                    <table cellpadding="0" cellspacing="0" class="gray-tb">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th>옵션</th>
                                <td colspan="3">{{orderInfo.optionconts}}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    <div>
                                        ...
                                        <div class="img-with-text" style="width: 202px;" v-for="(row,n) in files.reviewimgfile.length > 5 ? 5 : files.reviewimgfile.length" :key="row">
                                    ...
                                    <div class="mt20">
                                        <div class="img-with-text" style="width: 202px;" v-for="(row,n) in files.reviewimgfile.length < 10 ? files.reviewimgfile.length >= 5 ? files.reviewimgfile.length % 5 : 0 : 5" :key="row+5">
                                            <div class="img-thumb size200 no-image" v-if="$util.isNull(files['reviewimgfile'][n+5])"></div>
                                            <div class="img-thumb size200" v-else>
                                                <img :src="imgPreview['reviewimgfile'][n+5]" :alt="'리뷰이미지'+n" v-if="!$util.isNull(files['reviewimgfile'][n+5])" @click="showPicture(n+5)">
                                            </div>
                                            <a :href="imgPreview['reviewimgfile'][n+5]" download>
                                                <button type="button" class="btn blue-line mt10" style="width: 100%;" v-if="!$util.isNull(files['reviewimgfile'][n+5])">다운로드</button>
                                            </a>
                                        </div>
                                    </div>
                                </td>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    <div class="img-with-text" style="width: 202px;">
                                        <div class="img-thumb size200 no-image" v-if="$util.isNull(files.reviewvideofile)"></div>
                                        <div class="img-thumb size200" v-else>
                                            <img :src="imgPreview.reviewvideofile" :alt="'리뷰동영상'" v-if="!$util.isNull(files.reviewvideofile)" @click="showVideo">
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            <tr v-if="isAdmin">
                                ...
                                <td>
                                    <div class="radio_wrap">
                                        <input type="radio" name="group01blind" id="group11blind" v-model="info.isdisplay" value="F"><label for="group11blind">지정</label>
                                        <input type="radio" name="group01blind" id="group12blind" v-model="info.isdisplay" value="T"><label for="group12blind">해지</label>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="bar-title" v-if="isAdmin">베스트 설정</div>
                <div class="form-area" v-if="isAdmin">
                    <table cellpadding="0" cellspacing="0" class="gray-tb">
                        <colgroup>
                            <col width="150px">
                ...
                <div class="bar-title" v-if="isAdmin">신고 정보</div>
                <div class="scroll-y"  v-if="isAdmin" style="max-height: 300px;">
                ...
                <div class="btn-group">
                    <button type="button" class="btn big blue" v-if="isAdmin" @click="onSave">저장</button>
...
export default {
    name: 'admin.operation.review.detail',
    props: ['reviewidx'],
    components: {
        AdminMemberInfo,
        GoodsDetail,
    ...
    methods: {
        ...
        setInit() {
            ...
            if(this.info.isbest === 'T') {
                this.checkObj.startYYYYMMDD = this.$util.getFormatDate(this.info.beststtime.substring(0,8),'-');
                this.checkObj.startHH = this.info.beststtime.substring(8,10);
                this.checkObj.startMi = this.info.beststtime.substring(10,12);
                this.checkObj.endYYYYMMDD = this.$util.getFormatDate(this.info.bestedtime.substring(0,8),'-');
                this.checkObj.endHH = this.info.bestedtime.substring(8,10);
                this.checkObj.endMi = this.info.bestedtime.substring(10,12);
            } else {
            ...
            if(!this.$util.isNull(this.files['reviewvideofile'])){
                this.imgPreview.reviewvideofile = this.$util.changeFileType(this.files.reviewvideofile.fullpath,'.jpg');
            }
        ...
        onSave() {
            ...
            if(this.info.isbest === 'T'){ 
                this.info.beststtime = this.checkObj.startYYYYMMDD.replaceAll('-','') + this.checkObj.startHH + this.checkObj.startMi;
                if(this.info.beststtime.length < 12) {
                    alert("전시시작기간을 선택해주세요.");
                    return false;
                }

                this.info.bestedtime = this.checkObj.endYYYYMMDD.replaceAll('-','') + this.checkObj.endHH + this.checkObj.endMi;
                if(this.info.bestedtime.length < 12) {
                    alert("전시종료기간을 선택해주세요.");
                    return false;
                }
                if(this.info.beststtime >= this.info.bestedtime) {
                    alert("전시종료기간을 전시시작기간 이후로 선택해주세요.");
                    return false;
                }

                params.beststtime = this.info.beststtime;
                params.bestedtime = this.info.bestedtime;
            }

            this.$http.post('/admin/operation/review/check', params)
            .then(result => {
                if(result.statusCode === 200) {
