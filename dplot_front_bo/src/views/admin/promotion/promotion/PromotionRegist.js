...
export default {
    name: 'admin.promotion.promotion.promotionregist',
    components: {
    ...
    data() {
        return {
            ...
            info: {
                ...
                goodsrangetype: this.$store.getters['ADMIN'].GOODS_RANGE_ALL,   // 대상상품범위
                ispercent: '',          // 정률여부
                disprice: '',           // 할인액
                dispercent: '',         // 할인율
                partratio: '0',         // 파트너부담비율
                dadaratio: '100',       // 다다픽부담비율
        ...
        save: function() {
            if (this.checkValidation()) {
                ...
                if(confirm('저장하시겠습니까?')) {
                    ...
                    this.$http.post('/admin/promotion/promotion/save', params)
                        .then(result => {
                            this.$util.debug(result);
    ...
    watch: {
        ...
        'info.goodsrangetype': function(value) {
            if(value == this.$store.getters['ADMIN'].GOODS_RANGE_ALL) {
                this.promogoodsList = [];
            }
        },
        // 정률여부
