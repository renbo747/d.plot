              ...
              <dt>사용여부<i class="essential"></i></dt>
                <dd>
                    <div class="radio_wrap wide">
                        <input type="radio" name="group02" id="group21" v-model="boardInfo.istrash" value="F" checked/><label for="group21">사용함</label>
                        <input type="radio" name="group02" id="group22" v-model="boardInfo.istrash" value="T" /><label for="group22">사용안함</label>
                    </div>
                </dd>
              </dl>
            <dl>
...
export default {
  name: "admin.promotion.event.announce.detail",
  ...
  methods: {
    ...
    onSearchEnter() {
      ...
      this.$http.post("/admin/promotion/event/announce/search/enter", params)
          .then(result => {
    ...
    onSave() {
      ...
      if (confirm("저장 하시겠습니까?")) {
        ...
        this.removeEnterList.forEach(obj => {
          obj.eventidx = this.boardInfo.eventidx;
          obj.isfail = 'T'
        });
        ...
        let insertEnter = [];
        this.enterList.forEach(obj => {
          let isExist = this.dbEnterList.find(element => element.userid === obj.userid);
          if (typeof isExist === 'undefined') { // 중복 되지 않은 객체
            insertEnter.push(obj);
          }
        })
        ...
        this.removeEnterList.forEach(obj => {
          ...
          if (typeof isExist === 'undefined') { // 중복 되는 객체
            deleteEnter.push(obj);
          }
        })

        params.insertlist = insertEnter;
        params.removelist = deleteEnter;
        params.idx = this.idx;
    ...
    onSearch() {
      ...
      this.$http.post("/admin/promotion/event/announce/detail", params)
          .then(result => {
            if (result.statusCode === 200) {
              ...
              this.boardInfo.oldeventidx = this.boardInfo.eventidx;
    ...
    goUserDelete() {
      ...
      this.enterList.forEach(obj => {
        ...
        if (typeof isExist === 'undefined') {
          ...
          this.removeEnterList.push(obj);
        }
      })
      this.enterList = removedEnterList;

      // 삭제 당첨자 중복 제거
      this.removeEnterList = this.removeEnterList.filter((element, idx, arr) => {
