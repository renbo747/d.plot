<template>
    ...
    <div class="content m-leftmenu">
        ...
        <div class="inner">
            <div class="clearfix">
                <div class="bar-title fl">취소관리</div>
            </div>
            <div class="boxing search-area pd0">
                ...
                <dl>
                    ...
                    <dd>
                        <select v-model="searchData.dtkey">
                            <option value="clmreq">취소신청일</option>
                            <option value="clmcomp">취소완료일</option>
                ...
                <dl>
                    ...
                    <dd>
                        <div class="check-wrap">
                            <input type="checkbox" id="chkAllCncstatus" v-model="searchData.isallcncstatus" true-value="T" false-value="F" @change="checkAllCncstatus">
                            <label for="chkAllCncstatus">전체</label>
                        </div>
                        <div class="check-wrap" v-for="item in commonCode.cncstatus" :key="item.cmcode">
                            <input type="checkbox" :id="'cncstatus_'+item.cmcode" v-model="searchData.cncstatusArr" :true-value="[]" :value="item.cmcode"/>
                            <label :for="'cncstatus_'+item.cmcode">{{ item.codename }}</label>
                        </div>
                    </dd>
                </dl>
            </div>
            ...
            <div class="caption-group mt10 clearfix">
                ...
                <div class="btn-group fr">
                    <button type="button" v-if="isWrite" class="btn blue-line" @click="fnChangeCncstatus($store.getters['ADMIN'].CNC_COMP_APPRV, '취소승인')">취소승인</button>
                ...
                <table cellpadding="0" cellspacing="0" class="data-tb align-c" :style="isPartner? 'width: 1900px;': 'width: 2400px;'">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>취소상태
                                <button type="button" class="sort" :value="sortData.cncstatus"
                                    :class="[{up : sortData.cncstatus === 'cncstatus_asc'}, {down : sortData.cncstatus === 'cncstatus_desc'}]"
                                    @click="sortToggle(sortData.cncstatus)"></button>
                            </th>
                            <th>주문번호
                    ...
                    <tbody v-if="list.length>0">
                        <tr v-for="item in list" :key="item.clmgdidx">
                            ...
                            <td><a href="javascript:void(0);" class="link" @click="goCancelDetail(item)">{{ item.clmno }}</a></td>
                            ...
                            <td><a href="javascript:void(0);" class="link" @click="goClaimHistory(item.clmgdidx)">{{ item.cncstatusname }}</a></td>
        ...
        <CancelAdminDetail v-if="isShowCancelAdminDetail" :activeClmParam="activeClmParam" :activeOrderInfo="activeOrderInfo" @closeDetail="closeCancelAdminDetail"></CancelAdminDetail>
        <CancelPartnerDetail v-if="isShowCancelPartnerDetail" :activeClmParam="activeClmParam" :activeOrderInfo="activeOrderInfo" @closeDetail="closeCancelPartnerDetail"></CancelPartnerDetail>
...
import CancelAdminDetail from '@views.admin/order/claim/CancelAdminDetail.vue';
import CancelPartnerDetail from '@views.admin/order/claim/CancelPartnerDetail.vue';
...
export default {
    ...
    data() {
        return {
            ...
            searchData: {
                ...
                gdkey: '',              // 상품검색KEY
                gdword: '',             // 상품검색어
                dtkey: 'clmreq',        // 조회일자KEY (default: 취소신청일)
                period: '',             // 조회일자기간
                startdate: '',          // 조회시작일자
                enddate: '',            // 조회종료일자
                isallclmreqtype: '',    // 전체신청구분여부
                clmreqtypeArr: [],      // 신청구분Array
                isallcncstatus: '',     // 전체취소상태여부
                cncstatusArr: [],       // 취소상태Array
                dealerno: '',           // 입점업체번호
                psort: 'clmreqdate_desc'// 정렬조건 (default: 최근취소신청일)
            },
            commonCode: {
                clmreqtype: [],         // 클레임신청구분
                cncstatus: []           // 취소상태
            ...
            isShowCancelAdminDetail: false,  // 클레임취소상세 팝업 노출여부
            isShowCancelPartnerDetail: false, // 클레임취소상세 팝업 노출여부
    ...
    methods: {
        ...
        initSearchData: function () {
            this.searchData.clmtype = this.$store.getters['ADMIN'].CLM_CANCEL;
            this.searchData.skeyArr = [
                ...
                {key: 'clmno', name: '취소번호', isShowPartner: true}
            ];
            ...
            this.searchData.isallcncstatus = 'T';

            this.checkAllCncstatus();
        ...
        getCommonCodeList: function() {
            let cmclassArr = ['CLMREQTYPE', 'CNCSTATUS'];
            this.$http.post('/common/code/map/list', {cmclass : cmclassArr, isloading: false})
                .then(result =>{
                    ...
                    if(this.isLink){
                      ...
                      this.searchData.cncstatusArr = [linkParam.cncstatus];
                    }
                    // 목록 조회
                    this.searchList();
                })
        ...
        // 조회조건 - 취소상태 전체 체크
        checkAllCncstatus: function() {
            let isAllCheck = this.searchData.isallcncstatus;
            this.searchData.cncstatusArr = [];
            if (isAllCheck == 'T') {
                for(let type of this.commonCode.cncstatus){
                    this.searchData.cncstatusArr.push(type.cmcode);
                }
            }
        },
        // 조회조건 - 신청구분 전체 체크
        ...
        // 취소승인
        fnChangeCncstatus: function(reqStatus, reqName) {
            ...
            switch(reqStatus) {
                // 승인완료(승인대기)
                case this.$store.getters['ADMIN'].CNC_COMP_APPRV:
                    possibleStatus = [
            ...
            for (let i=0; i<this.list.length; i++) {
                for (let j=0; j<this.checkedList.length; j++) {
                    ...
                    if (this.list[i].clmgdidx === clmgdidx) {
                        let cncstatus = this.list[i].cncstatus;
                        if (!this.$util.isNull(possibleStatus) && possibleStatus.length > 0 && possibleStatus.indexOf(cncstatus) < 0) {
        ...
        goCancelDetail: function(obj) {
            ...
            if (this.isPartner) {
                ...
                this.isShowCancelPartnerDetail = true;
            } else {
                this.activeClmParam = {
                    orderidx: obj.orderidx,
                    clmidx: obj.clmidx
                }
                this.isShowCancelAdminDetail = true;
            }
        },
        // 클레임 Admin 상세 팝업 닫기
        closeCancelAdminDetail: function(isreload) {
            this.isShowCancelAdminDetail = false;
            if (isreload) {
                this.searchList();
            }
        },
        // 클레임 Partner 상세 팝업 닫기
        closeCancelPartnerDetail: function(isreload) {
            this.isShowCancelPartnerDetail = false;
            if (isreload) {
                this.searchList();
            }
        },
    ...
    watch: {
        ...
        // 취소상태
        'searchData.cncstatusArr': function(value) {
            if (value.length < this.commonCode.cncstatus.length) {
                this.searchData.isallcncstatus = 'F';
            } else {
                this.searchData.isallcncstatus = 'T';
            }
        },
