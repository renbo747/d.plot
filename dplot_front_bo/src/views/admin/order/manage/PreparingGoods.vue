<template>
    ...
    <div class="content m-leftmenu">
        ...
        <div class="inner">
            <div class="clearfix">
                <div class="bar-title fl">상품준비중</div>
            </div>
            ...
            <div class="caption-group mt10 clearfix">
                <div class="total-group fl">
                    ...
                    <span v-if="!isPartner">상품준비중 <strong>{{ count.prepgoodscnt }}</strong>건</span>
                </div>
                <div class="btn-group fr">
                    <button type="button" v-if="isWrite" class="btn blue-line" @click="fnChangeOrdStatus($store.getters['ADMIN'].ORDER_PREPARING_DELIV)">배송준비중</button>
            ...
            <div :class="{'scroll-x': !isPartner}">
                <table cellpadding="0" cellspacing="0" class="data-tb align-c" :style="!isPartner? 'width: 2575px;':''">
                    ...
                    <tbody v-else>
                        <tr><td :colspan="isPartner? 18:29">조회 결과가 존재하지 않습니다.</td></tr>
                    </tbody>
                </table>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            count: {                // 데이터 건수
                totalcnt: 0,        // 전체 건수
                comppaycnt: 0,      // 결제완료 건수
                prepgoodscnt: 0     // 상품준비중 건수
            },
            checkedList: [],        // 선택된 목록
            isRead : false,
            isWrite : false,
    ...
    methods: {
        ...
        getCommonCodeList: function() {
            let cmclassArr = ['MUAPPCHTYPE', 'ORDSTATUS', 'DADAMEMBERTYPE', 'MEMLVTYPE', 'PAYWAYTYPE'];
            this.$http.post('/common/code/map/list', {cmclass : cmclassArr, isloading: false})
                .then(result =>{
                    ...
                    for (const [key] of Object.entries(data)) {
                        ...
                        if (key === 'ordstatus') {
                            this.commonCode[key].splice(0, 2);
                            this.commonCode[key].splice(2);
                        }
                    }
                    // 검색조건 초기화
                    this.initSearchData();
        ...
        fnExcelDownload: function() {
            ...
            params.isPartner = this.isPartner;
            params.fileName = '상품준비중_주문목록.xlsx';
            ...
            for (let i=0; i<this.checkedList.length; i++) {
                ...
                for (let j=0; j<this.list.length; j++) {
                    ...
                    if (orgdelividx === item.orgdelividx) {
                        ...
                        if (item.ispbgoods === 'T') {
                            alert('파트너사 상품만 배송준비중 처리가 가능합니다. 확인후 진행해주세요.');
                            return;
                        }
                        if (item.ordstatus != this.$store.getters['ADMIN'].ORDER_PREPARING_GOODS) {
