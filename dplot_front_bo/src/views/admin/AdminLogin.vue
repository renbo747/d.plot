...
<script>
import SecondAuth from '@views/admin/SecondAuth.vue';
export default {
  name: 'AdminLogin',
  data () {
    return {
  ...
  methods: {
    login: function(){
        ...
        this.$http.post('/admin/login_act', param).then(result => {
          ...
          if(result.status === 200) {
            this.result = result;
            if(result.id ==='admin' || process.env.NODE_ENV === 'local' || process.env.NODE_ENV === 'dev'){
              this.successLogin();
            }
            else{
              this.openSecondAuth();
    ...
    successLogin: function(){
      this.$storage.setLocalStorage(this.$store.getters.CONSTANTS.ADMIN_USER, this.result);
