                        ...
                        <dl>
                            ...
                            <dd>
                                <div class="radio_wrap wide">
                                    <input type="radio" v-model="info.istrash" name="dgroup00" id="dgroup01" value="F" checked/><label for="dgroup01">사용함</label>
                                    <input type="radio" v-model="info.istrash" name="dgroup00" id="dgroup02" value="T" /><label for="dgroup02">사용안함</label>
                                </div>
                            </dd>
                        </dl>
                        <dl>
...
<script>
export default {
    name: 'admin.configuration.manage.notifydetail',
    props: ['idx'],
    components: {
    ...
    methods: {
        onInit() {
            ...
            this.$http.post('admin/configuration/manage/notify/detail', param)
            .then(result => {
        ...
        onSave() {
            ...
            if(this.removeItemList.length > 0){
                params.removeitemlist = this.removeItemList;
            }
            
            if(this.checkValidation()) {
                if(confirm("저장하시겠습니까?")){
                    this.$http.post("/admin/configuration/manage/notify/modify", params)
                    .then(result => {
                        if(result.statusCode === 200) {
                            alert("저장이 완료되었습니다.");
                            this.onClose(true);
        ...
        removeItem(index) {
            ...
            if(!this.$util.isNull(this.itemList[index].notifyitemidx)) {
                let idx = this.itemList[index].notifyitemidx;
                this.removeItemList.push(idx);
            }
