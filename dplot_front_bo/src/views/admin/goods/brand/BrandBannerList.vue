<template>
    <div class="content m-leftmenu">
        ...
        <BrandBannerRegist v-if="showRegist" v-bind:commonCodeList="commonCode" :brandname="searchData.brandname" :brandidx="searchData.brandidx" v-on:closeRegist="closeRegist"></BrandBannerRegist>
        <BrandBannerDetail v-if="showDetail" :commonCodeList="commonCode" :banidx="idx" v-on:closeDetail="closeDetail"></BrandBannerDetail>
                ...
                <dl>
                    ...
                    <dd>
                        <input type="search" style="width: 220px;" placeholder="브랜드명을 검색하세요." :maxlength="200"
                            v-model="searchData.brandname" @keyup.enter="openSearchBrandPopup"
                            @change="searchData.brandidx = ''" />
...
import BrandBannerRegist from "./BrandBannerRegist";
import BrandBannerDetail from "./BrandBannerDetail";
export default {
    ...
    mounted() {
        // if(!this.$util.isNull(this.$route.params.brandidx)) {
        //     let menuUrl = this.$options.name;
        //     let sideMenu = this.$parent._data.sideMenu;
        //     let thisMenuObj;
        //     sideMenu.forEach(parent => {
        //         if(parent.childmenulist != null && parent.childmenulist.length > 0) {
        //             parent.childmenulist.forEach(child => {
        //                 if(child.url === menuUrl) {
        //                     thisMenuObj = child;
        //                 }
        //             })
        //         }
        //     })
        //     this.$parent.onActive(thisMenuObj);
        // }
    ...
    methods: {
        ...
        initSearchData: function () {
            ...
            this.searchData.brandidx = this.$route.params.brandidx;
            this.searchData.brandname = this.$route.params.brandname;
        ...
        // 검색
        searchList(page) {
            if(page === 1 && this.$util.isNull(this.searchData.brandidx)) {
                return;
            }

            let param = this.searchData;

            if(this.$util.isNull(param.brandidx)) {
               alert("브랜드를 선택해주세요.");
               return; 
            }

            this.$http.post("/admin/goods/brand/banner/search", param)
            .then(result => {
                let data = result.data;
                this.list = data.list;
                this.state = data.state;
                this.$util.dataSetSearchParam(this);
                this.nowbrandidx = this.searchData.brandidx;
                this.isallchk = false;
        ...
        showChange(show) {
            ...
            if(show === 'F') { 
                ...
                this.$http.post("/admin/goods/brand/banner/check", params)
                .then(result => {
        ...
        update(params) {
            if(confirm(params.msg)) {
                this.$http.post("/admin/goods/brand/banner/update", params)
                .then(result => {
                    if (result.statusCode == 200) {
                        alert("저장이 완료되었습니다.");
                        this.searchList();
        ...
        openSearchBrandPopup: function() {
            let param = { brandname: this.searchData.brandname };
            this.$eventBus.$emit('modalShow', SearchBrandListPopup, param,
                (result) => {
                    this.searchData.brandidx = result.idx;
                    this.searchData.brandname = result.brandname;
                }
            );
        },
        goRegist() {
        ...
        closeRegist(isreload, brandidx, brandname) {
            this.showRegist = false;
            if(isreload) {
                this.searchData.brandidx = brandidx;
                this.searchData.brandname = brandname;
                this.searchList();
            }
        },
        goDetail(idx) {
