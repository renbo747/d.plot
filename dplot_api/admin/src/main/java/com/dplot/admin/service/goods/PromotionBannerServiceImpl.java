...
/**
 * @discription	: 프로모션배너 ServiceImpl
 * @fileName	: PromotionBannerServiceImpl.java
 * @author		: LKW
 * @date		: 2022.04.14
 * =================================================================
 * DATE			AUTHOR		NOTE
 ...
 */
@Service
public class PromotionBannerServiceImpl extends MallBaseService implements PromotionBannerService {
    private static final Logger logger = LoggerFactory.getLogger(PromotionBannerServiceImpl.class);
    ...
    @Override
    public SOMap selectPromotionBannerList(SOMap params) throws Exception {
		params.put("siteid", cs.getStr("siteid"));

		SOMap result = new SOMap();
		params.put("bntype", "BNT005");
		// TODO: 프로모션배너 이미지타입 생성시 변경필요
		params.put("pcimgtype", CMConst.IMG_TYPE_BRAND_BANNER_PC);
		params.put("moimgtype", CMConst.IMG_TYPE_BRAND_BANNER_MO);
    ...
    public SOMap updatePromotionBanner(SOMap params) throws Exception {
    	SOMap result = new SOMap();
	...
	@Override
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class, CustomException.class})
	public SOMap savePromotionBanner(SOMap params, Map<String, MultipartFile> uploadFile) throws Exception {
		SOMap result = new SOMap();
		...
		try {
			...
			if(uploadFile.containsKey("pcimgfile")){
                ...
                SOMap s3Result = fileService.uploadImage(pcimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_BRAND_BANNER_PC);
                result.put(CMConst.IMG_TYPE_BRAND_BANNER_PC, s3Result);
            }
            if(uploadFile.containsKey("mobileimgfile")){
                MultipartFile mobileimgFile = uploadFile.get("mobileimgfile");
                SOMap s3Result = fileService.uploadImage(mobileimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_BRAND_BANNER_MO);
                result.put(CMConst.IMG_TYPE_BRAND_BANNER_MO, s3Result);
            }
		} catch(Exception e){
	        logger.error(e.getMessage());
	        throw new CustomException("프로모션배너 등록시 오류가 발생하였습니다. 관리자에게 문의해주세요.");
	    }
	...
	@Override
	public SOMap selectPromotionBannerDetail(SOMap params) throws Exception {
		SOMap result = new SOMap();
		
		params.put("siteid", cs.getStr("siteid"));
		
		SOMap fileParams = new SOMap();
		String[] imgtypes = {CMConst.IMG_TYPE_BRAND_BANNER_PC,CMConst.IMG_TYPE_BRAND_BANNER_MO};
		...
		fileList.forEach(m -> {
			if(CMConst.IMG_TYPE_BRAND_BANNER_PC.equals(m.get("imgtype"))){
				files.put("pcimgfile", m);
			}else if(CMConst.IMG_TYPE_BRAND_BANNER_MO.equals(m.get("imgtype"))){
				files.put("mobileimgfile", m);
			}
		});
		
		SOMap info = bannerMapper.selectMainBannerDetail(params);
		
		SOMap tempParam = new SOMap();
		tempParam.put("idx", info.getStr("brandidx"));
		tempParam.put("siteid", cs.getStr("siteid"));
		SOMap brandInfo = brandMapper.selectBrandDetail(tempParam);
		
		info.put("brandname", brandInfo.getStr("name"));
	...
	public SOMap modifyPromotionBanner(SOMap params, Map<String, MultipartFile> uploadFile) throws Exception {
		...
		try {
            ...
            if(uploadFile.containsKey("pcimgfile")){
                MultipartFile pcimgFile = uploadFile.get("pcimgfile");
                SOMap s3Result = fileService.uploadImage(pcimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_BRAND_BANNER_PC);
                resultMap.put(CMConst.IMG_TYPE_BRAND_BANNER_PC, s3Result);
            }
            if(uploadFile.containsKey("mobileimgfile")){
                MultipartFile mobileimgFile = uploadFile.get("mobileimgfile");
                SOMap s3Result = fileService.uploadImage(mobileimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_BRAND_BANNER_MO);
                resultMap.put(CMConst.IMG_TYPE_BRAND_BANNER_MO, s3Result);
            }
            ...
            if(params.containsKey("copycheck")) {
                ...
                SOMap s3Result = fileService.uploadImage(multipartFile, params.getInt("banidx"), CMConst.IMG_TYPE_BRAND_BANNER_MO);
            }
            bannerMapper.updateMainBanner(params);
            
            bannerMapper.updateAllSortNum(params);
		} catch(Exception e){
        	throw new CustomException("프로모션배너 수정시 오류가 발생하였습니다. 관리자에게 문의해주세요.");
        }
	...
	@Override
	public SOMap checkPromotionBanner(SOMap params) throws Exception {
		params.put("siteid", cs.getStr("siteid"));
		params.put("bntype", "BNT005");
		SOMap result = new SOMap();
