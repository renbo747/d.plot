...
public class ExternalController {
    ...
    public Response sendERPData(@RequestBody SOMap param) throws Exception {
        ...
        } else if("DDMC_SHOP_RETURN_UPDATE2".equals(param.getStr("type"))){
            //직매입 반품상태수정(교환/반품)
            erpService.receiveReturnDeliveryInfoERPData(param);
        }
    ...
    public Response sendExternalBatch(@RequestBody SOMap param) throws Exception {
        ...
        } else if("META_EP".equals(param.getStr("type"))){
			commonService.createMetaEp(param);
        } else if("GOOGLE_EP".equals(param.getStr("type"))){
        	commonService.createGoogleEp(param);
        } else if("Reward".equals(param.getStr("type"))){
        	commonOrderService.saveRewardConfirmOrder(param);
        } else if("tempcode".equals(param.getStr("type"))){
        	cjMessageService.sendKakaoMsgReserve(param);
        }
