...
//		param.put("limit", 8);
		param.put("isshopmain", "T");
		List<SOMap> shopex1List = shopEx12Mapper.selectFrontShopExListView(param);
		result.put("shopex1list", shopex1List);
		/**************************
		 * 전시영역2 조회 - New Arrivals
		 **************************/
		param.put("extype", "EXN002");
		param.put("limit", 18);
		param.remove("isshopmain");
		//List<SOMap> shopex2List = shopEx12Mapper.selectFrontShopExListView(param);		메인 페이지 부하를 줄이기 위해 사용하지 않는 전시영역2는 조회하지 않고 skip(James, 2022-10-11)
		result.put("shopex2list", shopex1List);
		/**************************
		 * 전시영역3 조회 - D.BRAND
		 **************************/
		param.put("limit", 2);
	...
	public SOMap selectShopBestList(SOMap param) throws Exception {
		...
		param.put("mainyn", "T"); // main화면에서는 카테고리 상관없이 전체 6개 보여지게 하기위해.
	...
	public SOMap selectFilterList(SOMap param) throws Exception {
		...
		// 20220928 필터 수정개발 후 혜택관련 사용안함. 불필요한 쿼리실행을 안하기 위해 주석처리
...
//		}
		 ...
		 * 선택된 카테고리의 추천 목록 조회
		 **********************/
		SOMap recominfo = goodsMapper.selectRecomListView(param);
		...
		if (recominfo != null) {
			...
			if (recominfo.getStr("issalebadge").equals("T")) {
				SOMap temp2 = new SOMap();
				temp2.put("id", "issalebadge");
				temp2.put("name", "SALE");
				temp2.put("checked", false);
				recomList.add(temp2);
			}
			if (recominfo.getStr("iscouponbadge").equals("T")) {
				SOMap temp2 = new SOMap();
				temp2.put("id", "iscouponbadge");
	...
	public SOMap selectShopNewList(SOMap param) throws Exception {
		...
		List<SOMap> shopNewList = shopEx12Mapper.selectFrontShopExListView(param);
	...
	@Override
	public SOMap selectShopIsLoading01(SOMap param) throws Exception {
		SOMap result = new SOMap();
		param.put("siteid", cs.getStr("siteid"));
		param.put("platform", cs.getStr("platform"));
		param.put("muappchtype", cs.getStr("platform"));
...
//		TimeUnit.SECONDS.sleep(5);
	...
	@Override
	public SOMap selectShopIsLoading04(SOMap param) throws Exception {
		SOMap result = new SOMap();
		param.put("siteid", cs.getStr("siteid"));
		param.put("platform", cs.getStr("platform"));
		param.put("muappchtype", cs.getStr("platform"));
		 ...
		 * 카테고리 배너 조회
		 **********************/
		param.put("imgtype", Util.flag2Bool(cs.getStr("ismobile")) ? CMConst.IMG_TYPE_SHOPPING_CATE_MO
				: CMConst.IMG_TYPE_SHOPPING_CATE_PC);
		param.put("bntype", "BNT003");
		param.put("exbntype", null);
		param.put("limit", null);
		List<SOMap> cateList = bannerMapper.selectFrontMainBanner(param);
	...
	@Override
	public SOMap selectShopIsLoadingOther(SOMap param) throws Exception {
...
//		param.put("limit", 8);
		...
		param.put("limit", 18);
		param.remove("isshopmain");
		// List<SOMap> shopex2List = shopEx12Mapper.selectFrontShopExListView(param); 메인
		// 페이지 부하를 줄이기 위해 사용하지 않는 전시영역2는 조회하지 않고 skip(James, 2022-10-11)
		result.put("shopex2list", shopex1List);
		/**************************
		 * 전시영역3 조회 - D.BRAND
		 **************************/
		...
		for (SOMap shop : shopex5List) {
			...
			if (param.getInt("ex45idx") != 0) {
				...
				for (SOMap item : goodsList) {
					item.put("keyword",
							"".equals(item.get("keyword")) ? new ArrayList<>() : item.getStr("keyword").split(","));
				}
				shop.put("goodslist", goodsList);
			}
		}
		...
		// 키워드 추출
		for (SOMap shop : mzTrendList) {
			shop.put("keyword", "".equals(shop.getStr("keywords")) ? new ArrayList<>()
					: ((String) shop.get("keywords")).split(","));
		}
		return result;
	}

	@Override
	public SOMap selectShopMain1(SOMap param) throws Exception {
		SOMap result = new SOMap();
		param.put("siteid", cs.getStr("siteid"));
		param.put("platform", cs.getStr("platform"));
		param.put("muappchtype", cs.getStr("platform"));
...
//		TimeUnit.SECONDS.sleep(5);
	...
	@Override
	public SOMap selectShopMain2(SOMap param) throws Exception {
		SOMap result = new SOMap();
		param.put("siteid", cs.getStr("siteid"));
		param.put("platform", cs.getStr("platform"));
		param.put("muappchtype", cs.getStr("platform"));
