	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		try {
			...
			/*if (!hashedPassword.equals(user.getPassword())) {
				...
				dealerAuthManagerService.increasePwdFailCnt(username);
				throw new BadCredentialsException("비밀번호가 일치하지 않습니다. ( 실패 횟수 [ " + (user.getPwfailcnt() + 1) + "/5 ] )");
			} else {
				dealerAuthManagerService.resetPwFailCnt(username);
			}*/
