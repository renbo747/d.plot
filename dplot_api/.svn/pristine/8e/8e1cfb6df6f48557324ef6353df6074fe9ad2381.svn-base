...
//	private OrderMapper orderMapper;
	...
	public SOMap goodsDetail(SOMap params) throws Exception {
		...
		} else if (!Util.isEmpty(detail) && (
				   (!detail.getDbStr("mumemlvtype").contains(params.getDbStr("mumemlvtype")) && !Util.isEmpty(detail.getDbStr("mumemlvtype"))) //회원등급
				|| (!detail.getDbStr("mumembertype").contains(params.getDbStr("mumembertype")) && !Util.isEmpty(detail.getDbStr("mumembertype"))) // 회원유형
				|| !detail.getDbStr("muappchtype").contains(params.getDbStr("muappchtype")) // 
				|| (detail.getDbStr("selltarget").equals("SP") && detail.getDbStr("sellyn").equals("N")) // 특정회원
				)) {
			throw new BizException("구매 제한 상품입니다.");
		}
