...
public class CommonOrderServiceImpl extends MallBaseService implements CommonOrderService{
	...
	public SOMap confirmOrder(SOMap param) throws Exception {
		...
		// 구매확정 카카오톡 발송
		param.put("goodsname", orderInfo.getDbStr("goodsname"));
		param.put("ordno", orderInfo.getDbStr("ordno"));
		param.put("orderdate", orderInfo.getDbStr("orderdt"));
		param.put("number", orderInfo.getDbStr("ordtel"));
		cjMessageService.sendConfirmOrder(param);
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class})
	public void saveRewardConfirmOrder(SOMap param) throws Exception {
		SOMap rewardInfo = recomRewardLogMapper.selectSignupRewardLog(param);
		/*************************************
		 * 추천인 리워드 체크.
		 *************************************/
		if (rewardInfo == null || rewardInfo.getDbStr("iscfmgive").equals("F")) {
			logger.info("추천인 리워드이벤트가 업습니다.");
			return;
		}
		 ...
		 * 첫구매확정 주문 체크
		 *************************************/
		param.put("stday", rewardInfo.getStr("recomstday"));
		param.put("edday", rewardInfo.getStr("recomedday"));
		param.put("relogidx", rewardInfo.getStr("relogidx"));

		List<SOMap> confirmList = comOrderGoodsDelivMapper.selectRewardConfirmOrder(param);
		if (confirmList == null) {
			logger.info("첫 구매확정 추천인 리워드 대상자가 없습니다.");
			return;
		}
		for (SOMap soMap : confirmList) {
			SOMap dbParam = new SOMap();
			String startDay = "";
			String endDay = "";
			dbParam.put("siteid", param.getStr("siteid"));
			dbParam.put("userno", soMap.getStr("recuserno"));
			
			SOMap userInfo = memberMapper.selectMemberInfoByUserNo(dbParam);
			if (userInfo == null) {
				logger.info("회원정보를 가져오는데에 실패하였습니다. 실패 userno : " + soMap.getStr("recuserno"));
				continue;
			}
			
			if (userInfo.getStr("name").equals("탈퇴회원")) {
				logger.info("Message", "탈퇴회원으로 리워드 적립대상에서 제외 userno : " + soMap.getStr("recuserno"));
				continue;
			}
			
			if (userInfo.getStr("name").equals("휴면회원")) {
				logger.info("Message", "휴면회원 리워드 적립대상에서 제외 userno :" + soMap.getStr("recuserno"));
				continue;
			}
			...
			if (confirmList != null) {
				...
				if (rewardInfo.getDbStr("recomtype").equals("RCT001")) {// 적립금
					...
					dbParam.put("ordgdidx", soMap.getStr("ordgdidx"));
					commonservice.paymentReserve(dbParam);
					
					logger.info("Message", soMap.getDbInt("userno") + "님에게 추천인구매확정 적립금 리워드가 지급되었습니다.");
				}
				/*********************
				 * 추천리워드 보상::epoint
				 *********************/
				if (rewardInfo.getDbStr("recomtype").equals("RCT002")) {
					...
					dbParam.put("epopaytype", "EPT004");
					dbParam.put("epopayreason", "피추천인구매확정보상");
					...
					logger.info("Message", soMap.getDbInt("userno") + "님에게 추천인구매확정 E포인트 리워드가 지급되었습니다.");
				}
				/*********************
				 * 추천리워드 보상::쿠폰
				 *********************/
				if (rewardInfo.getDbStr("recomtype").equals("RCT003")) {
					...
					// 피추천인 쿠폰지급
					dbParam.put("siteid", param.getStr("siteid"));
					dbParam.put("comcpnidx", rewardInfo.getStr("reccfmcpidx"));
					...
					logger.info("Message", soMap.getDbInt("userno") + "님에게 추천인구매확정 쿠폰 리워드가 지급되었습니다.");
				}
			}
		}
	}
	...
	public SOMap calOrderAmt(SOMap param) throws Exception {
		...
		for (SOMap item : items) {
			...
			item.put("realgoodsamt", (item.getDbInt("price") * item.getDbInt("ordcnt")) - item.getDbInt("salepromoamt")
					- item.getDbInt("goodscpnamt"));
			// 실제상품 공급가
...
//	                   }
					if ("DCT001".equals(item.getDbStr("delivfaretype"))) {
						// 무료
						item.put("delivamt", 0);
						if ("J".equals(param.getDbStr("isolatetype"))) { //제주 추가배송비
							item.put("adddelivamt", item.getDbInt("chejufare"));
						} else if ("I".equals(param.getDbStr("isolatetype"))) { //내륙 추가배송비
					...
					} else if ("DCT005".equals(item.getDbStr("delivfaretype"))) {
						// 구매 수량 비례
						int delivfare = item.getDbInt("delivfare");
						int ordcnt = item.getDbInt("ordcnt");
						item.put("delivamt", delivfare * ordcnt);
						if ("J".equals(param.getDbStr("isolatetype"))) { //제주 추가배송비
							item.put("adddelivamt", item.getDbInt("chejufare"));
						} else if ("I".equals(param.getDbStr("isolatetype"))) { //내륙 추가배송비
							item.put("adddelivamt", item.getDbInt("isolfare"));
						...
						if (idx == -1) {
							tempList.add(item.getDbInt("delividx"));
							boolean isOk = false;
							List<SOMap> test = items.stream()
									.filter(a -> a.getDbInt("delividx") == item.getDbInt("delividx")
											&& "T".equals(a.getDbStr("iscombdeliv")))
									.collect(Collectors.toList());
							long sum = 0;
							for (int i = 0; i < test.size(); i++) {
								Map<String, Long> temp = test.stream()
										.collect(Collectors.toMap(a -> a.get("goodsno").toString(),
												a -> Long.parseLong((a.get("realgoodsamt")).toString()), Long::sum));
								sum += test.get(i).getInt("realgoodsamt");
			...
			// 프로모션 할인금액 적용가
			int saleamt = item.getDbInt("tempsaleamt");

			// 배송비쿠폰
			item.put("delivcpnamt", 0);
			if (delivCoupon != null && delivCoupon.getDbInt("goodsno") == item.getDbInt("goodsno")
				...
				// 장바구니 쿠폰할인
				if (Util.flag2Bool(coupon.getDbStr("isorderlimit"))
						&& (saleamt * item.getDbInt("ordcnt")) < coupon.getDbInt("orlimitamt")) {
					disamount = 0;
				}
			...
			// 주문총금액
			totAmt.put("ordtotprice",
					totAmt.getDbInt("ordtotprice") + (item.getDbInt("price") * item.getDbInt("ordcnt")));
			// 총프로모션할인금액
			...
			if (item.getDbInt("dealerno") > 0) {
				totAmt.put("ptndelivamt",
						totAmt.getDbInt("ptndelivamt") + item.getDbInt("delivamt") + item.getDbInt("adddelivamt")); // 파트너
																													// 총배송비금액
			} else {
				totAmt.put("dadadelivamt",
						totAmt.getDbInt("dadadelivamt") + item.getDbInt("delivamt") + item.getDbInt("adddelivamt")); // 자사
																														// 총배송비금액
			}
		}
		...
		if (cartCoupon != null) {
			...
			// 전체 취소인 경우 할인 가격들을 제외해주어야 쿠폰을 되돌려주는 로직을 태움.
			if(totAmt.getDbInt("ordtotprice") != 0) {
				paytotprice = paytotprice - totAmt.getDbInt("totdelivcpnamt") - totAmt.getDbInt("totsalepromoamt") - totAmt.getDbInt("totgoodscpnamt");
			}
			
			disamount = disamount > paytotprice ? paytotprice : disamount;

			// 장바구니 쿠폰할인
			if (Util.flag2Bool(coupon.getDbStr("isorderlimit")) && totAmt.getDbInt("ordtotprice")
					- totAmt.getDbInt("totsalepromoamt") < coupon.getDbInt("orlimitamt")) {
				disamount = 0;
			}
		...
		totAmt.put("totcpnamt", totAmt.getDbInt("totsalepromoamt") + totAmt.getDbInt("totgoodscpnamt")
				+ totAmt.getDbInt("basketcpnamt") + totAmt.getDbInt("totdelivcpnamt"));
		// 결제총금액 (주문총금액 - 총할인금액 + 배송비)
		totAmt.put("paytotprice", totAmt.getDbInt("ordtotprice") - totAmt.getDbInt("totcpnamt")
				+ totAmt.getDbInt("dadadelivamt") + totAmt.getDbInt("ptndelivamt"));
		// 실제결제금액 (결제총금액 - 사용포인트)
		totAmt.put("rpaytotprice", totAmt.getDbInt("paytotprice") - param.getDbInt("reservetotamt")
				- param.getDbInt("epointtotamt") - param.getDbInt("empreservetotamt"));
		// 적립금계산 (주문총금액 - 할인금액 + 배송비)
		totAmt.put("reservecalamt", totAmt.getDbInt("ordtotprice") - (totAmt.getDbInt("totsalepromoamt") + totAmt.getDbInt("totgoodscpnamt") + totAmt.getDbInt("basketcpnamt")
		+ totAmt.getDbInt("totdelivcpnamt"))+ totAmt.getDbInt("dadadelivamt") + totAmt.getDbInt("ptndelivamt"));
