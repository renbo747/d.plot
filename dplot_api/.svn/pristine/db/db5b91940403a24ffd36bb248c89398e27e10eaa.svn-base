...
public class AdminClaimServiceImpl extends MallBaseService implements AdminClaimService {
    ...
    public SOMap selectClaimDetailInfo(SOMap params) throws Exception {
		...
		if (!Util.equal(params.getStr("clmtype"), CMConst.CLM_CANCEL)) {
			...
			delivParam.put("userno", claimGoods.get("dealerno"));
	    	result.put("rtnlogislist", dealerDelivMapper.selectPartnersDeliveryTemp(delivParam));
//	    	result.put("rtnlogislist", dealerDelivMapper.selectPartnersDeliveryList(delivParam));
...
//		            break;
	...
	public SOMap procReturn(SOMap params) throws Exception {
		...
		try {
			if (Util.equal(params.getStr("rtndelivtype"), CMConst.DELIV_DIRECT)) { // 직배송 회수
				// 클레임 회수지 갱신
				claimMapper.updateClaimRecExcdlvInfo(params);
				// 클레임 상태변경
				result = commonClaimService.saveClaimStatus(params);
            	...
            	sendParams.put("clmno", params.get("clmno"));
        ...
        try {
        	if (Util.equal(params.getStr("delivtype"), CMConst.DELIV_DIRECT)) {
    	        ...
    	        if (Util.equal(prop.getProperty("Globals.Profile"), "prod")) {
    	        	...
    	        	orderMap.put("payment_date", DateTimeUtil.getFormatStr(params.getStr("clmreqday"), DateTimeUtil.MALL_DATE_FORMAT_FULL_SHORT));
    	        } else {
				...
				if (!"200".equals(result.getStr("status"))) {
					...
					logger.debug(result.getStr("status"));
					logger.debug(result.getStr("message"));
					logger.debug(result.getStr("details"));
					logger.info("=============== 교환배송추적 요청중 오류 status ================ : " + result.getStr("status"));
					logger.info("=============== 교환배송추적 요청중 오류 message =============== : " + result.getStr("message"));
					logger.info("=============== 교환배송추적 요청중 오류 details =============== : " + result.getStr("details"));
					throw new CustomException(errorMsg);
				}
				params.put("istracking", "T");
	        	// 클레임 배송지 갱신
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class, CustomException.class})
	public SOMap procReturnMan(SOMap params) throws Exception {
		params.put("siteid", cs.getStr("siteid"));
		SOMap result = new SOMap();
		...
		try {
			...
			sendParams.put("trans_unique_code", transUniqueCd);
			sendParams.put("from_name", Util.equal(claimGoodsMap.getStr("ispbgoods"), "T")? cs.getStr("cfgsitename"):claimGoodsMap.getStr("dealername"));
			sendParams.put("logistics_code", deliveryTrackingService.getGoodsFlowLogisCode(params.getStr("reclogistype")));
			sendParams.put("invoice_no", params.get("recinvoiceno"));
			sendParams.put("dlvret_type", "D");
			sendParams.put("to_name", params.get("recname"));
			...
			result = deliveryTrackingService.sendTraceRequest(sendParams, sendOrderList);
			if (!"200".equals(result.getStr("status"))) {
				String errorMsg = String.format("%s(%s)", "수동 교환/반품 배송추적요청중 오류가 발생하였습니다. ", result.getStr("message"));
				throw new CustomException(errorMsg);
			}
			// 클레임 배송지 갱신
			params.put("isrectracking", "T");
			claimMapper.updateClaimRecExcdlvInfo(params);
			// 클레임 상태변경
			params.put("rtnstatus", CMConst.RETURN_IN_DELIVERY);
			params.put("excstatus", CMConst.EXCHANGE_RETURN_DELIVERY);
			result = commonClaimService.saveClaimStatus(params);
		} catch(CustomException e){
			logger.error(e.getMessage());
			throw e;
