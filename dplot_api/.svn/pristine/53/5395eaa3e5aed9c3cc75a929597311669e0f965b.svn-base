...
/**
 * @discription	: 쇼핑메인배너 ServiceImpl
 * @fileName	: ShoppingMainBannerServiceImpl.java
 * @author		: LKW
 * @date		: 2022.04.04
 * =================================================================
 * DATE			AUTHOR		NOTE
 ...
 */
@Service
public class ShoppingMainBannerServiceImpl extends MallBaseService implements ShoppingMainBannerService {
    private static final Logger logger = LoggerFactory.getLogger(ShoppingMainBannerServiceImpl.class);
    ...
    public SOMap selectMainBannerList(SOMap params) throws Exception {
		...
		params.put("bntype", "BNT002");
		params.put("pcimgtype", CMConst.IMG_TYPE_SHOPPING_MAIN_PC);
		params.put("moimgtype", CMConst.IMG_TYPE_SHOPPING_MAIN_MO);
	...
	public SOMap saveMainBanner(SOMap params, Map<String, MultipartFile> uploadFile) throws Exception {
		...
		try {
			...
			if(uploadFile.containsKey("pcimgfile")){
                ...
                SOMap s3Result = fileService.uploadImage(pcimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_SHOPPING_MAIN_PC);
                result.put(CMConst.IMG_TYPE_SHOPPING_MAIN_PC, s3Result);
            }
            if(uploadFile.containsKey("mobileimgfile")){
                MultipartFile mobileimgFile = uploadFile.get("mobileimgfile");
                SOMap s3Result = fileService.uploadImage(mobileimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_SHOPPING_MAIN_MO);
                result.put(CMConst.IMG_TYPE_SHOPPING_MAIN_MO, s3Result);
            }
		} catch(Exception e){
	        logger.error(e.getMessage());
	...
	public SOMap selectBannerDetail(SOMap params) throws Exception {
		...
		String[] imgtypes = {CMConst.IMG_TYPE_SHOPPING_MAIN_PC,CMConst.IMG_TYPE_SHOPPING_MAIN_MO};
		...
		fileList.forEach(m -> {
			if(CMConst.IMG_TYPE_SHOPPING_MAIN_PC.equals(m.get("imgtype"))){
				files.put("pcimgfile", m);
			}else if(CMConst.IMG_TYPE_SHOPPING_MAIN_MO.equals(m.get("imgtype"))){
	...
	public SOMap modifyBanner(SOMap params, Map<String, MultipartFile> uploadFile) throws Exception {
		...
		try {
            ...
            if(uploadFile.containsKey("pcimgfile")){
                MultipartFile pcimgFile = uploadFile.get("pcimgfile");
                SOMap s3Result = fileService.uploadImage(pcimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_SHOPPING_MAIN_PC);
                resultMap.put(CMConst.IMG_TYPE_SHOPPING_MAIN_PC, s3Result);
            }
            if(uploadFile.containsKey("mobileimgfile")){
                MultipartFile mobileimgFile = uploadFile.get("mobileimgfile");
                SOMap s3Result = fileService.uploadImage(mobileimgFile, params.getInt("banidx"), CMConst.IMG_TYPE_SHOPPING_MAIN_MO);
                resultMap.put(CMConst.IMG_TYPE_SHOPPING_MAIN_MO, s3Result);
            }
            ...
            if(params.containsKey("copycheck")) {
                ...
                SOMap s3Result = fileService.uploadImage(multipartFile, params.getInt("banidx"), CMConst.IMG_TYPE_SHOPPING_MAIN_MO);
            }
            params.put("siteid", cs.getStr("siteid"));
            params.put("bntype", "BNT002");
            bannerMapper.updateMainBanner(params);
